...
class MemcacheSettingsForm implements SettingsFormInterface
{
    /** @var array */
    ...
    public function getSections(DriverConfiguration $configuration)
    {
        ...
        return [
            [
                [
                    'title'  => 'speedy_memcache_persistent',
                    'fields' => [
                        'persistent' => [
                            'type'  => 'yes_no',
    ...
    public function validate(DriverConfiguration $configuration)
    {
        ...
        foreach ($validate as $row_id => $model) {
            ...
            $result = $this->validateArray($model, [
                'host'   => 'required',
                'port'   => 'required|integer|greaterThan[-1]',
                'weight' => 'integer',
            ]);
    ...
    private function getServersGrid(array $settings)
    {
        ...
        $grid->loadAssets();
        $grid->setColumns([
            'speedy_memcache_servers_host',
            'speedy_memcache_servers_port',
            'speedy_memcache_servers_weight',
        ]);
        $grid->setNoResultsText('speedy_memcache_servers_no_results', 'speedy_memcache_servers_add_btn');
