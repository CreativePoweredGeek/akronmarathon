...
class Seo_lite_tab {
    ...
    public function display($channel_id, $entry_id = '')
    {
        ...
        $publisher_installed = ee('Addon')->get('publisher') && ee('Addon')->get('publisher')->isInstalled();
        $audit_url = ee('CP/URL', 'addons/settings/seo_lite/audit_entry')
            ->setQueryStringVariable('entry_id', $entry_id);

        if ($publisher_installed) {
            $default_publisher_id = $this->getDefaultPublisherLanguageId();

            if ($default_publisher_id) {
                $audit_url->setQueryStringVariable('publisher_id', $default_publisher_id);
            }
        }
        ...
        // Normalize OG Type for legacy installs that stored the select index (0,1,2...) instead of the label.
        // EE select fields can store numeric indexes when field_list_items is an indexed array.
        if ($ogtype !== '' && $ogtype !== null && is_numeric($ogtype)) {
            $og_type_map = array(
                0 => 'Article',
                1 => 'Website',
                2 => 'Book',
            ...
            $ogtype_int = (int) $ogtype;
            if (isset($og_type_map[$ogtype_int])) {
                $ogtype = $og_type_map[$ogtype_int];
            }
        }
        ...
        $settings['seo_lite_og_type'] = array(
            ...
            'field_required' => 'n',
            'field_data' => $ogtype,
            'field_list_items' => array('Article' => 'Article', 'Website' => 'Website', 'Book' => 'Book', 'Music - Song' => 'Music - Song', 'Music - Album' => 'Music - Album', 'Music - Playlist' => 'Music - Playlist', 'Music - Radio Station' => 'Music - Radio Station', 'Profile' => 'Profile', 'Video - Movie' => 'Video - Movie', 'Video - Episode' => 'Video - Episode', 'Video - TV Show' => 'Video - TV Show', 'Video - Other' => 'Video - Other'),
            'field_fmt' => '',
        ...
        $settings['seo_lite_twitter_type'] = array(
            ...
            'field_list_items' => array('Summary' => 'Summary', 'Summary - Large Image' => 'Summary - Large Image', 'Player' => 'Player'),
            'field_fmt' => '',
    ...
    function save($channel_entry, $params)
    {
        ...
        // EE7 can omit some tab fields from $params in certain publish flows (live preview, revisions, etc.)
        // Fall back to POST to ensure select/file fields reliably persist.
        $getParam = function(string $key, $default = '') use ($params) {
            if (isset($params[$key])) {
                return $params[$key];
            }
        ...
        $content = array(
            'site_id' => $site_id,
            'entry_id' => $entry_id,
            'title' => $getParam('seo_lite_title', ''),
            'keywords' => $getParam('seo_lite_keywords', ''),
            'description' => $getParam('seo_lite_description', ''),
            'robots_directive' => $getParam('seo_lite_robots_directive', ''),
            'og_title' => $getParam('seo_lite_og_title', ''),
            'og_description' => $getParam('seo_lite_og_description', ''),
            'og_type' => $getParam('seo_lite_og_type', ''),
            'og_url' => $getParam('seo_lite_og_url', ''),
            'og_image' => $getParam('seo_lite_og_image', ''),
            'twitter_title' => $getParam('seo_lite_twitter_title', ''),
            'twitter_description' => $getParam('seo_lite_twitter_description', ''),
            'twitter_type' => $getParam('seo_lite_twitter_type', ''),
            'twitter_image' => $getParam('seo_lite_twitter_image', '')
        );
