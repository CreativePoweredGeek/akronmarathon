...
class Seo_lite
{
    ...
    private function perform()
    {
        ...
        if ($category_url_title) {
            ...
            if ($q->num_rows() > 0) {
                ...
                $twitter_image = $this->resolveImageUrl($this->get_preferred_value($seolite_entry->default_twitter_image, $default_twitter_image));
                $og_image = $this->resolveImageUrl($this->get_preferred_value($seolite_entry->default_og_image, $default_og_image));
    ...
    private function resolveImageUrl($file_reference)
    {
    ...
    {
        // Backward compatible:
        // - legacy versions stored 0-3 (int) in the DB
        // - EE7-safe tab builds may store the literal string like "NOINDEX, FOLLOW"
        if ($value === null) {
            return '';
        }
        ...
        if (is_numeric($value)) {
            switch ((int) $value) {
                case 0: return 'INDEX, FOLLOW';
                case 1: return 'NOINDEX, FOLLOW';
                case 2: return 'INDEX, NOFOLLOW';
    ...
    {
        // Backward compatible:
        // - legacy versions stored 0-11 (int) in the DB
        // - EE7-safe tab builds may store the Open Graph type slug (e.g., "website")
        //   or a human label (e.g., "Website").

        if ($value === null) {
            return '';
        }

        // 1) Legacy numeric storage
        if (is_numeric($value)) {
            switch ((int) $value) {
                case 0:  return 'article';
                case 1:  return 'website';
                case 2:  return 'book';
                case 3:  return 'music.song';
                case 4:  return 'music.album';
                case 5:  return 'music.playlist';
                case 6:  return 'music.radio_station';
                case 7:  return 'profile';
                case 8:  return 'video.movie';
                case 9:  return 'video.episode';
                case 10: return 'video.tv_show';
                case 11: return 'video.other';
                default: return '';
            }
        }

        // 2) Slug/label storage
    ...
    {
        // Backward compatible:
        // - legacy versions stored 0-2 (int) in the DB
        // - EE7-safe tab builds may store the slug (e.g., "summary_large_image") or a label
        if ($value === null) {
            return '';
        }
        ...
        if (is_numeric($value)) {
            switch ((int) $value) {
                ...
                case 1: return 'summary_large_image';
