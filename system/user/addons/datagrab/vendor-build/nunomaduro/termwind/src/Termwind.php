...
use BoldMinded\DataGrab\Dependency\Termwind\Exceptions\InvalidChild;
/**
 * @internal
 */
final class Termwind
{
    ...
    public static function ul(array $content = [], string $styles = '', array $properties = []) : Components\Ul
    {
        ...
        $content = self::prepareElements($content, $styles, static function ($li) use($ul) : string|Element {
            if (\is_string($li)) {
                return $li;
            }
    ...
    public static function ol(array $content = [], string $styles = '', array $properties = []) : Components\Ol
    {
        ...
        $index = 0;
        $content = self::prepareElements($content, $styles, static function ($li) use($ol, &$index) : string|Element {
            if (\is_string($li)) {
                return $li;
            }
            if (!$li instanceof Components\Li) {
    ...
    {
        $content = self::prepareElements($content, $styles, static function ($element) : string|Element {
