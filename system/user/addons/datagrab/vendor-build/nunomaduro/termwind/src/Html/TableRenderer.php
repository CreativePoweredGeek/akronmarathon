...
namespace BoldMinded\DataGrab\Dependency\Termwind\Html;
...
use BoldMinded\DataGrab\Dependency\Symfony\Component\Console\Helper\TableCellStyle;
...
use BoldMinded\DataGrab\Dependency\Termwind\HtmlRenderer;
use BoldMinded\DataGrab\Dependency\Termwind\Termwind;
use BoldMinded\DataGrab\Dependency\Termwind\ValueObjects\Node;
...
final class TableRenderer
{
    ...
    private function parseHeader(Node $node) : void
    {
        ...
        if ($title !== '') {
            $this->table->getStyle()->setHeaderTitleFormat($this->parseTitleStyle($node));
    ...
    private function parseFoot(Node $node) : void
    {
        $title = $node->getAttribute('title');
        if ($title !== '') {
            $this->table->getStyle()->setFooterTitleFormat($this->parseTitleStyle($node));
    ...
    private function parseRow(Node $node) : Iterator
    {
        ...
        foreach ($node->getChildNodes() as $child) {
            if ($child->isName('th') || $child->isName('td')) {
                ...
                $text = (string) (new HtmlRenderer())->parse(\trim(\preg_replace('/<br\\s?+\\/?>/', "\n", $child->getHtml()) ?? ''));
                ...
                $row[] = new TableCell(
                    ...
                    [
                        ...
                        'style' => $this->parseCellStyle($class, $align === '' ? TableCellStyle::DEFAULT_ALIGN : $align),
                    ]
                );
            }
        }
