...
namespace BoldMinded\DataGrab\Dependency\Termwind\Actions;

use BoldMinded\DataGrab\Dependency\Termwind\Exceptions\StyleNotFound;
use BoldMinded\DataGrab\Dependency\Termwind\Repositories\Styles as StyleRepository;
use BoldMinded\DataGrab\Dependency\Termwind\Terminal;
...
final class StyleToMethod
{
    ...
    {
        $stylesString = self::sortStyles(\array_merge($styles->defaultStyles(), \array_filter((array) \preg_split('/(?![^\\[]*\\])\\s/', $stylesString))));
    ...
    public function __invoke(string|int ...$arguments) : Styles
    {
        ...
        $method = \array_filter((array) \preg_split('/(?![^\\[]*\\])-/', $method), fn($item) => $item !== \false);
        ...
        return $this->styles->setStyle($this->style)->{$methodName}(...\array_reverse($arguments));
    }
    /**
    ...
    private static function sortStyles(array $styles) : array
    {
        ...
        \usort($styles, function ($a, $b) use($keys) {
            ...
            if ($existsA && \array_search($matchesA[1], $keys, \true) > \array_search($matchesB[1], $keys, \true)) {
                return 1;
            }
            return -1;
        });
