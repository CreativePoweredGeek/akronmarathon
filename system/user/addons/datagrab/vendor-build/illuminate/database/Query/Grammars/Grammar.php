...
namespace BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\Grammars;

use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Database\Query\Expression;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Concerns\CompilesJsonPaths;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Grammar as BaseGrammar;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\Builder;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\JoinClause;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\JoinLateralClause;
...
class Grammar extends BaseGrammar
{
    ...
    public function compileSelect(Builder $query)
    {
        ...
        $sql = \trim($this->concatenate($this->compileComponents($query)));
        if ($query->unions) {
    ...
    protected function compileJoins(Builder $query, $joins)
    {
        return (new Collection($joins))->map(function ($join) use($query) {
            ...
            return \trim("{$join->type} join {$tableAndNestedJoins} {$this->compileWheres($join)}");
        })->implode(' ');
    }
    /**
    ...
    {
        return (new Collection($query->wheres))->map(fn($where) => $where['boolean'] . ' ' . $this->{"where{$where['type']}"}($query, $where))->all();
    }
    /**
    ...
    {
        $over = \trim('partition by ' . $this->wrap($partition) . ' ' . $orders);
    ...
    public function compileInsert(Builder $query, array $values)
    {
        ...
        $parameters = (new Collection($values))->map(fn($record) => '(' . $this->parameterize($record) . ')')->implode(', ');
    ...
    public function compileUpdate(Builder $query, array $values)
    {
        ...
        return \trim(isset($query->joins) ? $this->compileUpdateWithJoins($query, $table, $columns, $where) : $this->compileUpdateWithoutJoins($query, $table, $columns, $where));
    }
    /**
    ...
    {
        return (new Collection($values))->map(fn($value, $key) => $this->wrap($key) . ' = ' . $this->parameter($value))->implode(', ');
    }
    /**
    ...
    public function compileDelete(Builder $query)
    {
        $table = $this->wrapTable($query->from);
        $where = $this->compileWheres($query);
        return \trim(isset($query->joins) ? $this->compileDeleteWithJoins($query, $table, $where) : $this->compileDeleteWithoutJoins($query, $table, $where));
    }
    /**
