...
class MySqlProcessor extends Processor
{
    ...
    public function processIndexes($results)
    {
        return \array_map(function ($result) {
            $result = (object) $result;
            return ['name' => $name = \strtolower($result->name), 'columns' => $result->columns ? \explode(',', $result->columns) : [], 'type' => \strtolower($result->type), 'unique' => (bool) $result->unique, 'primary' => $name === 'primary'];
        }, $results);
    }
    /** @inheritDoc */
