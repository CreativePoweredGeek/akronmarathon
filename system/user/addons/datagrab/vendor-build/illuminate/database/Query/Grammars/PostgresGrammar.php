...
class PostgresGrammar extends Grammar
{
     ...
     * @var bool
     */
    protected static $cascadeTruncate = \true;
    /**
    ...
    {
        return \trim("{$join->type} join lateral {$expression} on true");
    }
    /**
    ...
    public function compileUpdateFrom(Builder $query, $values)
    {
        ...
        if (isset($query->joins)) {
            ...
            $froms = (new Collection($query->joins))->map(fn($join) => $this->wrapTable($join->table))->all();
            if (\count($froms) > 0) {
                $from = ' from ' . \implode(', ', $froms);
            }
        }
        $where = $this->compileUpdateWheres($query);
        return \trim("update {$table} set {$columns}{$from} {$where}");
    }
    /**
    ...
    protected function wrapJsonPathAttributes($path)
    {
        ...
        return (new Collection($path))->map(fn($attribute) => $this->parseJsonPathArrayKeys($attribute))->collapse()->map(function ($attribute) use($quote) {
    ...
    {
        static::$customOperators = \array_values(\array_merge(static::$customOperators, \array_filter(\array_filter($operators, 'is_string'))));
    }
    /**
     ...
     * @return void
     */
    public static function cascadeOnTruncate(bool $value = \true)
    {
     ...
     */
    public static function cascadeOnTrucate(bool $value = \true)
    {
