...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\ModelPruningFinished;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\ModelPruningStarting;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\ModelsPruned;
...
#[\Symfony\Component\Console\Attribute\AsCommand(name: 'model:prune')]
class PruneCommand extends Command
{
    ...
    protected function pruneModel(string $model)
    {
        ...
        $total = $model::isPrunable() ? $instance->pruneAll($chunkSize) : 0;
        if ($total == 0) {
    ...
    protected function models()
    {
        ...
        if ($models) {
            return (new Collection($models))->filter(static fn(string $model) => \class_exists($model))->values();
        }
        return (new Collection(Finder::create()->in($this->getPath())->files()->name('*.php')))->map(function ($model) {
            $namespace = $this->laravel->getNamespace();
            return $namespace . \str_replace(['/', '.php'], ['\\', ''], Str::after($model->getRealPath(), \realpath(app_path()) . \DIRECTORY_SEPARATOR));
        })->when(!empty($except), fn($models) => $models->reject(fn($model) => \in_array($model, $except)))->filter(fn($model) => $this->isPrunable($model))->values();
    }
    /**
    ...
    protected function getPath()
    {
        if (!empty($path = $this->option('path'))) {
            return (new Collection($path))->map(fn($path) => base_path($path))->all();
        }
    ...
    {
        $instance = new $model();
        $count = $instance->prunable()->when($model::isSoftDeletable(), function ($query) {
            $query->withTrashed();
        })->count();
        if ($count === 0) {
            $this->components->info("No prunable [{$model}] records found.");
    ...
    {
        return \class_exists($model) && \is_a($model, Model::class, \true) && !(new \ReflectionClass($model))->isAbstract() && $model::isPrunable();
    }
}
