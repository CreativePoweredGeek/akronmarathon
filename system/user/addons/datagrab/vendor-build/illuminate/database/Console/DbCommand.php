...
#[\Symfony\Component\Console\Attribute\AsCommand(name: 'db')]
class DbCommand extends Command
{
    ...
    public function handle()
    {
        ...
        try {
            (new Process(\array_merge([$command = $this->getCommand($connection)], $this->commandArguments($connection)), null, $this->commandEnvironment($connection)))->setTimeout(null)->setTty(\true)->mustRun(function ($type, $buffer) {
    ...
    {
        $optionalArguments = ['password' => '--password=' . $connection['password'], 'unix_socket' => '--socket=' . ($connection['unix_socket'] ?? ''), 'charset' => '--default-character-set=' . ($connection['charset'] ?? '')];
        if (!$connection['password']) {
            unset($optionalArguments['password']);
        }
        return \array_merge(['--host=' . $connection['host'], '--port=' . $connection['port'], '--user=' . $connection['username']], $this->getOptionalArguments($optionalArguments, $connection), [$connection['database']]);
    }
    /**
