...
#[\Symfony\Component\Console\Attribute\AsCommand(name: 'migrate:status')]
class StatusCommand extends BaseCommand
{
    ...
    public function handle()
    {
        return $this->migrator->usingConnection($this->option('database'), function () {
            ...
            $migrations = $this->getStatusFor($ran, $batches)->when($this->option('pending') !== \false, fn($collection) => $collection->filter(function ($migration) {
    ...
    {
        return (new Collection($this->getAllMigrationFiles()))->map(function ($migration) use($ran, $batches) {
    ...
    {
        return [['database', null, InputOption::VALUE_OPTIONAL, 'The database connection to use'], ['pending', null, InputOption::VALUE_OPTIONAL, 'Only list pending migrations', \false], ['path', null, InputOption::VALUE_OPTIONAL | InputOption::VALUE_IS_ARRAY, 'The path(s) to the migrations files to use'], ['realpath', null, InputOption::VALUE_NONE, 'Indicate any provided migration file paths are pre-resolved absolute paths']];
    }
}
