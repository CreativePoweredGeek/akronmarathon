...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Console\Seeds\WithoutModelEvents;
    ...
    public function call($class, $silent = \false, array $parameters = [])
    {
        ...
        foreach ($classes as $class) {
            ...
            if ($silent === \false && isset($this->command)) {
                (new TwoColumnDetail($this->command->getOutput()))->render($name, '<fg=yellow;options=bold>RUNNING</>');
            }
            $startTime = \microtime(\true);
            $seeder->__invoke($parameters);
            if ($silent === \false && isset($this->command)) {
                $runTime = \number_format((\microtime(\true) - $startTime) * 1000);
                (new TwoColumnDetail($this->command->getOutput()))->render($name, "<fg=gray>{$runTime} ms</> <fg=green;options=bold>DONE</>");
