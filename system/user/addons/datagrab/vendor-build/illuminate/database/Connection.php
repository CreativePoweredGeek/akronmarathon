...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\QueryExecuted;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\StatementPrepared;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\TransactionBeginning;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\TransactionCommitted;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\TransactionCommitting;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\TransactionRolledBack;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\Grammars\Grammar as QueryGrammar;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\Processors\Processor;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Schema\Builder as SchemaBuilder;
...
class Connection implements ConnectionInterface
{
     ...
     * @return array
     */
    public function selectResultSets($query, $bindings = [], $useReadPdo = \true)
    {
        return $this->run($query, $bindings, function ($query, $bindings) use($useReadPdo) {
            if ($this->pretending()) {
                return [];
     ...
     * @return \PDO
     */
    protected function getPdoForSelect($useReadPdo = \true)
    {
    ...
    public function logQuery($query, $bindings, $time = null)
    {
        ...
        $query = $this->pretending === \true ? $this->queryGrammar?->substituteBindingsIntoRawSql($query, $bindings) ?? $query : $query;
     ...
     * @return void
     */
    public function recordsHaveBeenModified($value = \true)
    {
     ...
     * @return $this
     */
    public function useWriteConnectionWhenReading($value = \true)
    {
    ...
    {
        $config = $this->latestReadWriteTypeUsed() === 'read' ? $this->readPdoConfig : $this->config;
        return ['driver' => $this->getDriverName(), 'name' => $this->getNameWithReadWriteType(), 'host' => $config['host'] ?? null, 'port' => $config['port'] ?? null, 'database' => $config['database'] ?? null, 'unix_socket' => $config['unix_socket'] ?? null];
    }
    /**
