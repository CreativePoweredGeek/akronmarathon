...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\QueryException;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Arr;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Facades\File;
class SQLiteBuilder extends Builder
{
    ...
    public function getTables($schema = null)
    {
        ...
        if (\version_compare($this->connection->getServerVersion(), '3.37.0', '<')) {
            ...
            foreach (Arr::wrap($schema) as $name) {
                $tables = \array_merge($tables, $this->connection->selectFromWriteConnection($this->grammar->compileLegacyTables($name, $withSize)));
            }
            return $this->connection->getPostProcessor()->processTables($tables);
        }
        return $this->connection->getPostProcessor()->processTables($this->connection->selectFromWriteConnection($this->grammar->compileTables($schema, $withSize)));
    }
    /** @inheritDoc */
    public function getViews($schema = null)
    {
        ...
        $views = [];
        foreach (Arr::wrap($schema) as $name) {
            $views = \array_merge($views, $this->connection->selectFromWriteConnection($this->grammar->compileViews($name)));
        }
    ...
    public function getColumns($table)
    {
        ...
        return $this->connection->getPostProcessor()->processColumns($this->connection->selectFromWriteConnection($this->grammar->compileColumns($schema, $table)), $this->connection->scalar($this->grammar->compileSqlCreateStatement($schema, $table)));
    }
    /**
    ...
    public function dropAllTables()
    {
        foreach ($this->getCurrentSchemaListing() as $schema) {
            $database = $schema === 'main' ? $this->connection->getDatabaseName() : (\array_column($this->getSchemas(), 'path', 'name')[$schema] ?: ':memory:');
