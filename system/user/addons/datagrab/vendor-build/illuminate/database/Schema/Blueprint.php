...
namespace BoldMinded\DataGrab\Dependency\Illuminate\Database\Schema;

use Closure;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Connection;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Concerns\HasUlids;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\Expression;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Schema\Grammars\Grammar;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Schema\Grammars\MySqlGrammar;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Schema\Grammars\SQLiteGrammar;
...
class Blueprint
{
    ...
    {
        return (new Collection($this->commands))->filter(fn($command) => \in_array($command->name, $names));
    }
    /**
    ...
    protected function addFluentIndexes()
    {
        foreach ($this->columns as $column) {
            foreach (['primary', 'unique', 'index', 'fulltext', 'fullText', 'spatialIndex', 'vectorIndex'] as $index) {
                ...
                if ($column->{$index} === \true) {
                    $indexMethod = $index === 'index' && $column->type === 'vector' ? 'vectorIndex' : $index;
    ...
    public function addAlterCommands()
    {
        ...
        [$commands, $lastCommandWasAlter, $hasAlterCommand] = [[], \false, \false];
        foreach ($this->commands as $command) {
            if (\in_array($command->name, $alterCommands)) {
                $hasAlterCommand = \true;
                $lastCommandWasAlter = \true;
    ...
    {
        return $this->unsignedInteger($column, \true);
    }
    /**
     * Create a new auto-incrementing integer column on the table (4-byte, 0 to 4,294,967,295).
     *
    ...
    {
        return $this->unsignedTinyInteger($column, \true);
    }
    /**
    ...
    {
        return $this->unsignedSmallInteger($column, \true);
    }
    /**
    ...
    {
        return $this->unsignedMediumInteger($column, \true);
    }
    /**
     * Create a new auto-incrementing big integer column on the table (8-byte, 0 to 18,446,744,073,709,551,615).
     *
    ...
    {
        return $this->unsignedBigInteger($column, \true);
    }
    /**
    ...
    {
        return $this->integer($column, $autoIncrement, \true);
    }
    /**
    ...
    {
        return $this->tinyInteger($column, $autoIncrement, \true);
    }
    /**
    ...
    {
        return $this->smallInteger($column, $autoIncrement, \true);
    }
    /**
    ...
    {
        return $this->mediumInteger($column, $autoIncrement, \true);
    }
    /**
    ...
    {
        return $this->bigInteger($column, $autoIncrement, \true);
    }
    /**
     * Create a new unsigned big integer column on the table (8-byte, 0 to 18,446,744,073,709,551,615).
     *
    ...
    public function foreignIdFor($model, $column = null)
    {
        ...
        if ($model->getKeyType() === 'int') {
            return $this->foreignId($column)->table($model->getTable())->referencesModelColumn($model->getKeyName());
        }
        $modelTraits = class_uses_recursive($model);
        if (\in_array(HasUlids::class, $modelTraits, \true)) {
            return $this->foreignUlid($column, 26)->table($model->getTable())->referencesModelColumn($model->getKeyName());
        }
        return $this->foreignUuid($column)->table($model->getTable())->referencesModelColumn($model->getKeyName());
    }
    /**
    ...
    {
        return new Collection([$this->timestampTz('created_at', $precision)->nullable(), $this->timestampTz('updated_at', $precision)->nullable()]);
    }
    /**
     * Add nullable creation and update timestampTz columns to the table.
     *
    ...
    public function numericMorphs($name, $indexName = null, $after = null)
    {
        ...
        $this->unsignedBigInteger("{$name}_id")->after(!\is_null($after) ? "{$name}_type" : null);
    ...
    public function nullableNumericMorphs($name, $indexName = null, $after = null)
    {
        ...
        $this->unsignedBigInteger("{$name}_id")->nullable()->after(!\is_null($after) ? "{$name}_type" : null);
        $this->index(["{$name}_type", "{$name}_id"], $indexName);
    }
    /**
    ...
    protected function indexCommand($type, $columns, $index, $algorithm = null, $operatorClass = null)
    {
        ...
        return $this->addCommand($type, \compact('index', 'columns', 'algorithm', 'operatorClass'));
    }
    /**
    ...
    protected function createIndexName($type, array $columns)
    {
        ...
        if ($this->connection->getConfig('prefix_indexes')) {
            $table = \str_contains($this->table, '.') ? \substr_replace($this->table, '.' . $this->connection->getTablePrefix(), \strrpos($this->table, '.'), 1) : $this->connection->getTablePrefix() . $this->table;
        }
