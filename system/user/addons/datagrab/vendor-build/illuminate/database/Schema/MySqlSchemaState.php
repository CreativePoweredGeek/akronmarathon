...
use BoldMinded\DataGrab\Dependency\Symfony\Component\Process\Exception\ProcessFailedException;
...
class MySqlSchemaState extends SchemaState
{
    ...
    {
        $this->executeDumpProcess($this->makeProcess($this->baseDumpCommand() . ' --routines --result-file="${:LARAVEL_LOAD_PATH}" --no-data'), $this->output, \array_merge($this->baseVariables($this->connection->getConfig()), ['LARAVEL_LOAD_PATH' => $path]));
    ...
    {
        $process = $this->executeDumpProcess($this->makeProcess($this->baseDumpCommand() . ' ' . $this->getMigrationTable() . ' --no-create-info --skip-extended-insert --skip-routines --compact --complete-insert'), null, \array_merge($this->baseVariables($this->connection->getConfig()), []));
    ...
    protected function connectionString(array $versionInfo)
    {
        ...
        if (isset($config['options'][\PHP_VERSION_ID >= 80500 ? \BoldMinded\DataGrab\Dependency\Pdo\Mysql::ATTR_SSL_CA : \PDO::MYSQL_ATTR_SSL_CA])) {
            $value .= ' --ssl-ca="${:LARAVEL_LOAD_SSL_CA}"';
        }
        /** @phpstan-ignore class.notFound */
        if (isset($config['options'][\PHP_VERSION_ID >= 80500 ? \BoldMinded\DataGrab\Dependency\Pdo\Mysql::ATTR_SSL_CERT : \PDO::MYSQL_ATTR_SSL_CERT])) {
            $value .= ' --ssl-cert="${:LARAVEL_LOAD_SSL_CERT}"';
        }
        /** @phpstan-ignore class.notFound */
        if (isset($config['options'][\PHP_VERSION_ID >= 80500 ? \BoldMinded\DataGrab\Dependency\Pdo\Mysql::ATTR_SSL_KEY : \PDO::MYSQL_ATTR_SSL_KEY])) {
            $value .= ' --ssl-key="${:LARAVEL_LOAD_SSL_KEY}"';
        }
        /** @phpstan-ignore class.notFound */
        $verifyCertOption = \PHP_VERSION_ID >= 80500 ? \BoldMinded\DataGrab\Dependency\Pdo\Mysql::ATTR_SSL_VERIFY_SERVER_CERT : \PDO::MYSQL_ATTR_SSL_VERIFY_SERVER_CERT;
    ...
    protected function baseVariables(array $config)
    {
        ...
        return [
            ...
            'LARAVEL_LOAD_SSL_CA' => $config['options'][\PHP_VERSION_ID >= 80500 ? \BoldMinded\DataGrab\Dependency\Pdo\Mysql::ATTR_SSL_CA : \PDO::MYSQL_ATTR_SSL_CA] ?? '',
            // @phpstan-ignore class.notFound
            'LARAVEL_LOAD_SSL_CERT' => $config['options'][\PHP_VERSION_ID >= 80500 ? \BoldMinded\DataGrab\Dependency\Pdo\Mysql::ATTR_SSL_CERT : \PDO::MYSQL_ATTR_SSL_CERT] ?? '',
            // @phpstan-ignore class.notFound
            'LARAVEL_LOAD_SSL_KEY' => $config['options'][\PHP_VERSION_ID >= 80500 ? \BoldMinded\DataGrab\Dependency\Pdo\Mysql::ATTR_SSL_KEY : \PDO::MYSQL_ATTR_SSL_KEY] ?? '',
        ];
    }
    /**
    ...
    protected function executeDumpProcess(Process $process, $output, array $variables, int $depth = 0)
    {
        ...
        try {
            ...
            if (Str::contains($e->getMessage(), ['column-statistics', 'column_statistics'])) {
                return $this->executeDumpProcess(Process::fromShellCommandLine(\str_replace(' --column-statistics=0', '', $process->getCommandLine())), $output, $variables, $depth + 1);
            }
            if (\str_contains($e->getMessage(), 'set-gtid-purged')) {
                return $this->executeDumpProcess(Process::fromShellCommandLine(\str_replace(' --set-gtid-purged=OFF', '', $process->getCommandLine())), $output, $variables, $depth + 1);
            }
            throw $e;
        }
        return $process;
    ...
    protected function detectClientVersion() : array
    {
        ...
        return ['version' => $version, 'isMariaDb' => $isMariaDb];
    }
}
