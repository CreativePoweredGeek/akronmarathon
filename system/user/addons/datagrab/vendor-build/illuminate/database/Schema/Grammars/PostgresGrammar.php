...
class PostgresGrammar extends Grammar
{
    ...
    {
        return 'select nspname as name, nspname = current_schema() as "default" from pg_namespace where ' . $this->compileSchemaWhereClause(null, 'nspname') . ' order by nspname';
    }
    /**
    ...
    {
        return \sprintf('select exists (select 1 from pg_class c, pg_namespace n where ' . "n.nspname = %s and c.relname = %s and c.relkind in ('r', 'p') and n.oid = c.relnamespace)", $schema ? $this->quoteString($schema) : 'current_schema()', $this->quoteString($table));
    }
    /**
    ...
    {
        return 'select c.relname as name, n.nspname as schema, pg_total_relation_size(c.oid) as size, ' . "obj_description(c.oid, 'pg_class') as comment from pg_class c, pg_namespace n " . "where c.relkind in ('r', 'p') and n.oid = c.relnamespace and " . $this->compileSchemaWhereClause($schema, 'n.nspname') . ' order by n.nspname, c.relname';
    }
    /**
    ...
    {
        return 'select viewname as name, schemaname as schema, definition from pg_views where ' . $this->compileSchemaWhereClause($schema, 'schemaname') . ' order by schemaname, viewname';
    }
    /**
    ...
    public function compileAutoIncrementStartingValues(Blueprint $blueprint, Fluent $command)
    {
        if ($command->column->autoIncrement && ($value = $command->column->get('startingValue', $command->column->get('from')))) {
            return \sprintf('select setval(pg_get_serial_sequence(%s, %s), %s, false)', $this->quoteString($this->wrapTable($blueprint)), $this->quoteString($command->column->name), $value);
        }
    }
    /** @inheritDoc */
    public function compileChange(Blueprint $blueprint, Fluent $command)
    {
        ...
        return \sprintf('alter table %s %s', $this->wrapTable($blueprint), \implode(', ', $this->prefixArray('alter column ' . $this->wrap($column), $changes)));
    }
    /**
    ...
    public function compileUnique(Blueprint $blueprint, Fluent $command)
    {
        ...
        if ($command->online || $command->algorithm) {
            $createIndexSql = \sprintf('create unique index %s%s on %s%s (%s)', $command->online ? 'concurrently ' : '', $this->wrap($command->index), $this->wrapTable($blueprint), $command->algorithm ? ' using ' . $command->algorithm : '', $this->columnize($command->columns));
            $sql = \sprintf('alter table %s add constraint %s unique using index %s', $this->wrapTable($blueprint), $this->wrap($command->index), $this->wrap($command->index));
        } else {
            $sql = \sprintf('alter table %s add constraint %s %s (%s)', $this->wrapTable($blueprint), $this->wrap($command->index), $uniqueStatement, $this->columnize($command->columns));
        }
    ...
    {
        return \sprintf('create index %s%s on %s%s (%s)', $command->online ? 'concurrently ' : '', $this->wrap($command->index), $this->wrapTable($blueprint), $command->algorithm ? ' using ' . $command->algorithm : '', $this->columnize($command->columns));
    }
    /**
    ...
    public function compileFulltext(Blueprint $blueprint, Fluent $command)
    {
        ...
        return \sprintf('create index %s%s on %s using gin ((%s))', $command->online ? 'concurrently ' : '', $this->wrap($command->index), $this->wrapTable($blueprint), \implode(' || ', $columns));
    }
    /**
    ...
    protected function compileIndexWithOperatorClass(Blueprint $blueprint, Fluent $command)
    {
        ...
        return \sprintf('create index %s%s on %s%s (%s)', $command->online ? 'concurrently ' : '', $this->wrap($command->index), $this->wrapTable($blueprint), $command->algorithm ? ' using ' . $command->algorithm : '', $columns);
    }
    /**
    ...
    {
        if (!$column->change && !$this->hasCommand($blueprint, 'primary') && (\in_array($column->type, $this->serials) || $column->generatedAs !== null) && $column->autoIncrement) {
            return ' primary key';
        }
    }
    /**
