...
namespace BoldMinded\DataGrab\Dependency\Illuminate\Database\Schema\Grammars;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Schema\ColumnDefinition;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Collection;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Fluent;
...
class MySqlGrammar extends Grammar
{
    ...
    {
        return 'select schema_name as name, schema_name = schema() as `default` from information_schema.schemata where ' . $this->compileSchemaWhereClause(null, 'schema_name') . ' order by schema_name';
    }
    /**
    ...
    {
        return \sprintf('select exists (select 1 from information_schema.tables where ' . "table_schema = %s and table_name = %s and table_type in ('BASE TABLE', 'SYSTEM VERSIONED')) as `exists`", $schema ? $this->quoteString($schema) : 'schema()', $this->quoteString($table));
    }
    /**
    ...
    {
        return 'select table_name as `name`, table_schema as `schema`, view_definition as `definition` ' . 'from information_schema.views where ' . $this->compileSchemaWhereClause($schema, 'table_schema') . ' order by table_schema, table_name';
    }
    /**
    ...
    protected function compileCreateTable($blueprint, $command)
    {
        ...
        if ($primaryKey = $this->getCommandByName($blueprint, 'primary')) {
            ...
            $primaryKey->shouldBeSkipped = \true;
        }
    ...
    {
        return \sprintf('alter table %s add %s%s%s', $this->wrapTable($blueprint), $this->getColumn($blueprint, $command->column), $command->column->instant ? ', algorithm=instant' : '', $command->column->lock ? ', lock=' . $command->column->lock : '');
    }
    /**
    ...
    public function compileRenameColumn(Blueprint $blueprint, Fluent $command)
    {
        ...
        if ($isMaria && \version_compare($version, '10.5.2', '<') || !$isMaria && \version_compare($version, '8.0.3', '<')) {
    ...
    {
        $column = (new Collection($this->connection->getSchemaBuilder()->getColumns($blueprint->getTable())))->firstWhere('name', $command->from);
        $modifiers = $this->addModifiers($column['type'], $blueprint, new ColumnDefinition(['change' => \true, 'type' => match ($column['type_name']) {
            'bigint' => 'bigInteger',
            'int' => 'integer',
        ...
        }, 'nullable' => $column['nullable'], 'default' => $column['default'] && (\str_starts_with(\strtolower($column['default']), 'current_timestamp') || $column['default'] === 'NULL') ? new Expression($column['default']) : $column['default'], 'autoIncrement' => $column['auto_increment'], 'collation' => $column['collation'], 'comment' => $column['comment'], 'virtualAs' => !\is_null($column['generation']) && $column['generation']['type'] === 'virtual' ? $column['generation']['expression'] : null, 'storedAs' => !\is_null($column['generation']) && $column['generation']['type'] === 'stored' ? $column['generation']['expression'] : null]));
        return \sprintf('alter table %s change %s %s %s', $this->wrapTable($blueprint), $this->wrap($command->from), $this->wrap($command->to), $modifiers);
    }
    /** @inheritDoc */
    public function compileChange(Blueprint $blueprint, Fluent $command)
    {
        $column = $command->column;
        $sql = \sprintf('alter table %s %s %s%s %s', $this->wrapTable($blueprint), \is_null($column->renameTo) ? 'modify' : 'change', $this->wrap($column), \is_null($column->renameTo) ? '' : ' ' . $this->wrap($column->renameTo), $this->getType($column));
    ...
    {
        return \sprintf('alter table %s add primary key %s(%s)%s', $this->wrapTable($blueprint), $command->algorithm ? 'using ' . $command->algorithm : '', $this->columnize($command->columns), $command->lock ? ', lock=' . $command->lock : '');
    }
    /**
    ...
    {
        return \sprintf('alter table %s add %s %s%s(%s)%s', $this->wrapTable($blueprint), $type, $this->wrap($command->index), $command->algorithm ? ' using ' . $command->algorithm : '', $this->columnize($command->columns), $command->lock ? ', lock=' . $command->lock : '');
    }
    /**
    ...
    protected function typeGeometry(Fluent $column)
    {
        ...
        return \sprintf('%s%s', $subtype ?? 'geometry', match (\true) {
