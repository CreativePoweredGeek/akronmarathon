...
namespace BoldMinded\DataGrab\Dependency\Illuminate\Database\Migrations;

use Closure;
use BoldMinded\DataGrab\Dependency\Illuminate\Console\View\Components\BulletList;
use BoldMinded\DataGrab\Dependency\Illuminate\Console\View\Components\Info;
use BoldMinded\DataGrab\Dependency\Illuminate\Console\View\Components\Task;
use BoldMinded\DataGrab\Dependency\Illuminate\Console\View\Components\TwoColumnDetail;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\MigrationEnded;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\MigrationsEnded;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\MigrationSkipped;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\MigrationsStarted;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\MigrationStarted;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Events\NoPendingMigrations;
...
class Migrator
{
    ...
    protected function pendingMigrations($files, $ran)
    {
        ...
        return (new Collection($files))->reject(fn($file) => \in_array($migrationName = $this->getMigrationName($file), $ran) || \in_array($migrationName, $migrationsToSkip))->values()->all();
    }
    /**
    ...
    {
        return (new Collection(self::$withoutMigrations))->map($this->getMigrationName(...))->all();
    }
    /**
    ...
    protected function pretendToRun($migration, $method)
    {
        ...
        $this->write(BulletList::class, (new Collection($this->getQueries($migration, $method)))->map(fn($query) => $query['query']));
    }
    /**
    ...
    {
        return (new Collection($paths))->flatMap(fn($path) => \str_ends_with($path, '.php') ? [$path] : $this->files->glob($path . '/*_*.php'))->filter()->values()->keyBy(fn($file) => $this->getMigrationName($file))->sortBy(fn($file, $key) => $key)->all();
    }
    /**
    ...
    public function resolveConnection($connection)
    {
        if (static::$connectionResolverCallback) {
            return \call_user_func(static::$connectionResolverCallback, $this->resolver, $connection ?: $this->connection);
        } else {
