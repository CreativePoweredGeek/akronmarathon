...
use BackedEnum;
use BoldMinded\DataGrab\Dependency\Brick\Math\BigDecimal;
use BoldMinded\DataGrab\Dependency\Brick\Math\Exception\MathException as BrickMathException;
use BoldMinded\DataGrab\Dependency\Brick\Math\RoundingMode;
use BoldMinded\DataGrab\Dependency\Carbon\CarbonImmutable;
use BoldMinded\DataGrab\Dependency\Carbon\CarbonInterface;
use DateTimeImmutable;
use DateTimeInterface;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Database\Eloquent\Castable;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Database\Eloquent\CastsInboundAttributes;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Support\Arrayable;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Casts\AsArrayObject;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Casts\AsCollection;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Casts\AsEncryptedArrayObject;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Casts\AsEncryptedCollection;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Casts\AsEnumArrayObject;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Casts\AsEnumCollection;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Casts\Attribute;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Casts\Json;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\InvalidCastException;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\JsonEncodingException;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\MissingAttributeException;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Relations\Relation;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\LazyLoadingViolationException;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Collection as BaseCollection;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Exceptions\MathException;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Facades\Hash;
...
use Stringable;
use ValueError;
use function BoldMinded\DataGrab\Dependency\Illuminate\Support\enum_value;
trait HasAttributes
{
    /**
     ...
     * @var bool
     */
    public static $snakeAttributes = \true;
    /**
    ...
    public function hasAttribute($key)
    {
        ...
        return \array_key_exists($key, $this->attributes) || \array_key_exists($key, $this->casts) || $this->hasGetMutator($key) || $this->hasAttributeMutator($key) || $this->isClassCastable($key);
    }
    /**
    ...
    protected function mutateAttributeForArray($key, $value)
    {
        ...
        } elseif (isset(static::$getAttributeMutatorCache[\get_class($this)][$key]) && static::$getAttributeMutatorCache[\get_class($this)][$key] === \true) {
            $value = $this->mutateAttributeMarkedAttribute($key, $value);
            $value = $value instanceof DateTimeInterface ? $this->serializeDate($value) : $value;
        } else {
    ...
    protected function castAttribute($key, $value)
    {
        ...
        switch ($castType) {
            ...
            case 'boolean':
                return (bool) $value;
            case 'object':
                return $this->fromJson($value, \true);
            case 'array':
            case 'json':
            case 'json:unicode':
    ...
    protected function getClassCastableAttributeValue($key, $value)
    {
        ...
        if (isset($this->classCastCache[$key]) && !$objectCachingDisabled) {
            ...
            $value = $caster instanceof CastsInboundAttributes ? $value : $caster->get($this, $key, $value, $this->attributes);
    ...
    {
        return $this->resolveCasterClass($key)->{$method}($this, $key, $value, $this->attributes);
    }
    /**
    ...
    {
        return $this->resolveCasterClass($key)->serialize($this, $key, $value, $this->attributes);
    }
    /**
    ...
    {
        return $this->resolveCasterClass($key)->compare($this, $key, $original, $value);
    }
    /**
    ...
    {
        return \in_array($key, $this->getDates(), \true) || $this->isDateCastable($key);
    }
    /**
    ...
    public function fillJsonAttribute($key, $value)
    {
        ...
        $value = $this->asJson($this->getArrayAttributeWithValue($path, $key, $value), $this->getJsonCastFlags($key));
    ...
    protected function setEnumCastableAttribute($key, $value)
    {
        ...
        if (!isset($value)) {
            ...
            $this->attributes[$key] = $this->getStorableEnumValue($enumClass, $this->getEnumCaseFromValue($enumClass, $value));
        }
    }
    /**
    ...
    protected function getStorableEnumValue($expectedEnum, $value)
    {
        if (!$value instanceof $expectedEnum) {
            throw new ValueError(\sprintf('Value [%s] is not of the expected enum type [%s].', \var_export($value, \true), $expectedEnum));
        }
    ...
    protected function getArrayAttributeByKey($key)
    {
        ...
        return $this->fromJson($this->isEncryptedCastable($key) ? $this->fromEncryptedString($this->attributes[$key]) : $this->attributes[$key]);
    }
    /**
    ...
    public function hasCast($key, $types = null)
    {
        if (\array_key_exists($key, $this->getCasts())) {
            return $types ? \in_array($this->getCastType($key), (array) $types, \true) : \true;
        }
        return \false;
    }
    /**
    ...
    {
        return !$this->isEnumCastable($key) && $this->isClassCastable($key) && \method_exists($this->resolveCasterClass($key), 'serialize');
    }
    /**
    ...
    {
        return !$this->isEnumCastable($key) && $this->isClassCastable($key) && \method_exists($this->resolveCasterClass($key), 'compare');
    }
    /**
    ...
    protected function mergeAttributeFromClassCasts(string $key) : void
    {
        ...
        $caster = $this->resolveCasterClass($key);
        $this->attributes = \array_merge($this->attributes, $caster instanceof CastsInboundAttributes ? [$key => $value] : $this->normalizeCastClassResponse($key, $caster->set($this, $key, $value, $this->attributes)));
    }
    /**
     * Merge the cast class attributes back into the model.
     *
    ...
    {
        return (new static())->setRawAttributes($this->original, $sync = \true)->getOriginalWithoutRewindingModel($key, $default);
    }
    /**
     * Get the model's original attribute values.
     *
    ...
    protected function getOriginalWithoutRewindingModel($key = null, $default = null)
    {
        ...
        return (new Collection($this->original))->mapWithKeys(fn($value, $key) => [$key => $this->transformModelValue($key, $value)])->all();
    }
    /**
    ...
    protected function transformModelValue($key, $value)
    {
        ...
        if ($this->hasCast($key)) {
            if (static::preventsAccessingMissingAttributes() && !\array_key_exists($key, $this->attributes) && ($this->isEnumCastable($key) || \in_array($this->getCastType($key), static::$primitiveCastTypes))) {
    ...
    public static function cacheMutatedAttributes($classOrInstance)
    {
        ...
        static::$getAttributeMutatorCache[$class] = (new Collection($attributeMutatorMethods = static::getAttributeMarkedMutatorMethods($classOrInstance)))->mapWithKeys(fn($match) => [\lcfirst(static::$snakeAttributes ? Str::snake($match) : $match) => \true])->all();
        static::$mutatorCache[$class] = (new Collection(static::getMutatorMethods($class)))->merge($attributeMutatorMethods)->map(fn($match) => \lcfirst(static::$snakeAttributes ? Str::snake($match) : $match))->all();
    }
    /**
