...
use ArrayAccess;
use Closure;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Broadcasting\HasBroadcastChannel;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\QueueableCollection;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\QueueableEntity;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Routing\UrlRoutable;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Support\Arrayable;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Support\CanBeEscapedWhenCastToString;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Support\Jsonable;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\ConnectionResolverInterface as Resolver;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Attributes\Boot;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Attributes\Initialize;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Attributes\Scope as LocalScope;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Attributes\UseEloquentBuilder;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Relations\HasManyThrough;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Relations\Pivot;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Stringable as SupportStringable;
...
abstract class Model implements Arrayable, ArrayAccess, CanBeEscapedWhenCastToString, HasBroadcastChannel, Jsonable, JsonSerializable, QueueableEntity, Stringable, UrlRoutable
{
    ...
    protected static function bootTraits()
    {
        ...
        foreach ((new ReflectionClass($class))->getMethods() as $method) {
            if (!\in_array($method->getName(), $booted) && $method->isStatic() && (\in_array($method->getName(), $conventionalBootMethods) || $method->getAttributes(Boot::class) !== [])) {
                $method->invoke(null);
     ...
     * @return void
     */
    public static function automaticallyEagerLoadRelationships($value = \true)
    {
    ...
    {
        return (new BaseCollection($columns))->map(fn($column) => $this->qualifyColumn($column))->all();
    }
    /**
    ...
    protected function resolveCustomBuilderClass()
    {
        ...
        return !empty($attributes) ? $attributes[0]->newInstance()->builderClass : \false;
    }
    /**
    ...
    {
        return \method_exists(static::class, $method) && (new ReflectionMethod(static::class, $method))->getAttributes(LocalScope::class) !== [];
    }
    /**
    ...
    {
        return $this->withoutRecursion(fn() => \array_merge($this->attributesToArray(), $this->relationsToArray()), fn() => $this->attributesToArray());
    }
    /**
    ...
    public function refresh()
    {
        ...
        $this->load((new BaseCollection($this->relations))->reject(fn($relation) => $relation instanceof Pivot || \is_object($relation) && \in_array(AsPivot::class, class_uses_recursive($relation), \true))->keys()->all());
        $this->syncOriginal();
        return $this;
    }
    /**
    ...
    {
        $defaults = \array_values(\array_filter([$this->getKeyName(), $this->getCreatedAtColumn(), $this->getUpdatedAtColumn(), ...$this->uniqueIds(), 'laravel_through_key']));
    ...
    public function __call($method, $parameters)
    {
        ...
        if (Str::startsWith($method, 'through') && \method_exists($this, $relationMethod = (new SupportStringable($method))->after('through')->lcfirst()->toString())) {
