...
use Closure;
use Exception;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Database\Eloquent\Builder as BuilderContract;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Database\Query\Expression;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Concerns\QueriesRelationships;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Database\RecordsNotFoundException;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\UniqueConstraintViolationException;
...
class Builder implements BuilderContract
{
     ...
     * @var string[]
     */
    protected $propertyPassthru = ['from'];
    /**
    ...
    {
        $this->withoutGlobalScopes(\array_diff(\array_keys($this->scopes), $scopes));
        return $this;
    }
    /**
    ...
    public function fillForInsert(array $values)
    {
        ...
        $this->model->unguarded(function () use(&$values) {
            foreach ($values as $key => $rowValues) {
                $values[$key] = tap($this->newModelInstance($rowValues), fn($model) => $model->setUniqueIds())->getAttributes();
            }
        });
    ...
    public function pluck($column, $key = null)
    {
        ...
        if (!$this->model->hasAnyGetMutator($column) && !$this->model->hasCast($column) && !\in_array($column, $this->model->getDates())) {
            return $this->applyAfterQueryCallbacks($results);
        }
        return $this->applyAfterQueryCallbacks($results->map(function ($value) use($column) {
    ...
    protected function ensureOrderForCursorPagination($shouldReverse = \false)
    {
        ...
        return (new BaseCollection($orders))->filter(fn($order) => Arr::has($order, 'direction'))->values();
    }
    /**
    ...
    public function upsert(array $values, $uniqueBy, $update = null)
    {
        ...
        return $this->toBase()->upsert($this->addTimestampsToUpsertValues($this->addUniqueIdsToUpsertValues($values)), $uniqueBy, $this->addUpdatedAtToUpsertColumns($update));
    }
    /**
    ...
    protected function addUpdatedAtColumn(array $values)
    {
        ...
        if (!\array_key_exists($column, $values)) {
            ...
            if ($this->model->hasSetMutator($column) || $this->model->hasAttributeSetMutator($column) || $this->model->hasCast($column)) {
                $timestamp = $this->model->newInstance()->forceFill([$column => $timestamp])->getAttributes()[$column] ?? $timestamp;
            }
    ...
    protected function groupWhereSliceForScope(QueryBuilder $query, $whereSlice)
    {
        ...
        if ($whereBooleans->contains(fn($logicalOperator) => \str_contains($logicalOperator, 'or'))) {
            $query->wheres[] = $this->createNestedWhere($whereSlice, \str_replace(' not', '', $whereBooleans->first()));
        } else {
    ...
    protected function prepareNestedWithRelationships($relations, $prefix = '')
    {
        ...
        foreach ($relations as $key => $value) {
            ...
            $preparedRelationships = \array_merge($preparedRelationships, ["{$prefix}{$attribute}" => $attributeSelectConstraint], $this->prepareNestedWithRelationships($value, "{$prefix}{$attribute}"));
            unset($relations[$key]);
        }
        ...
        foreach ($relations as $key => $value) {
            ...
            $preparedRelationships[$prefix . $key] = $this->combineConstraints([$value, $preparedRelationships[$prefix . $key] ?? static function () {
                //
            }]);
        }
    ...
    {
        return \str_contains($name, ':') ? $this->createSelectWithConstraint($name) : [$name, static function () {
            //
        }];
    }
    /**
    ...
    protected function createSelectWithConstraint($name)
    {
        return [\explode(':', $name)[0], static function ($query) use($name) {
            $query->select(\array_map(static function ($column) use($query) {
                return $query instanceof BelongsToMany ? $query->getRelated()->qualifyColumn($column) : $column;
     ...
     * @return $this
     */
    public function withAttributes(Expression|array|string $attributes, $value = null, $asConditions = \true)
    {
    ...
    {
        return $this->getQuery()->getConnection()->transactionLevel() > 0 ? $this->getQuery()->getConnection()->transaction($scope) : $scope();
    }
    /**
    ...
    {
        return isset($this->query->unions) ? (new BaseCollection($this->query->unions))->pluck('query') : new BaseCollection();
    }
    /**
    ...
    public static function __callStatic($method, $parameters)
    {
        ...
        if ($method === 'mixin') {
            return static::registerMixin($parameters[0], $parameters[1] ?? \true);
        }
        if (!static::hasGlobalMacro($method)) {
