...
trait CanBeOneOfMany
{
    ...
    public function ofMany($column = 'id', $aggregate = 'MAX', $relation = null)
    {
        ...
        foreach ($columns as $column => $aggregate) {
            ...
            $subQuery = $this->newOneOfManySubQuery($this->getOneOfManySubQuerySelectColumns(), \array_merge([$column], $previous['columns'] ?? []), $aggregate);
            if (isset($previous)) {
                $this->addOneOfManyJoinSubQuery($subQuery, $previous['subQuery'], $previous['columns']);
            }
            if (isset($closure)) {
                $closure($subQuery);
            }
            ...
            if (\array_key_last($columns) == $column) {
                $this->addOneOfManyJoinSubQuery($this->query, $subQuery, \array_merge([$column], $previous['columns'] ?? []));
            }
            $previous = ['subQuery' => $subQuery, 'columns' => \array_merge([$column], $previous['columns'] ?? [])];
        }
        $this->addConstraints();
