...
use Closure;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Builder;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Collection as EloquentCollection;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\RelationNotFoundException;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Relations\BelongsTo;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Relations\BelongsToMany;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Relations\MorphTo;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\Relations\Relation;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\Builder as QueryBuilder;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Query\Expression;
...
trait QueriesRelationships
{
    ...
    public function has($relation, $operator = '>=', $count = 1, $boolean = 'and', ?Closure $callback = null)
    {
        ...
        $hasQuery = $relation->{$method}($relation->getRelated()->newQueryWithoutRelationships(), $this);
    ...
    {
        return $this->whereHas(Str::before($relation, ':'), $callback, $operator, $count)->with($callback ? [$relation => fn($query) => $callback($query)] : $relation);
    }
    /**
    ...
    public function hasMorph($relation, $types, $operator = '>=', $count = 1, $boolean = 'and', ?Closure $callback = null)
    {
        ...
        $types = (array) $types;
        $checkMorphNull = $types === ['*'] && ($operator === '<' && $count >= 1 || $operator === '<=' && $count >= 0 || $operator === '=' && $count === 0 || $operator === '!=' && $count >= 1);
        if ($types === ['*']) {
            $types = $this->model->newModelQuery()->distinct()->pluck($relation->getMorphType())->filter()->map(fn($item) => enum_value($item))->all();
        }
        if (empty($types)) {
        ...
        return $this->where(function ($query) use($relation, $callback, $operator, $count, $types, $checkMorphNull) {
            foreach ($types as $type) {
                $query->orWhere(function ($query) use($relation, $callback, $operator, $count, $type) {
                    ...
                    $query->where($this->qualifyColumn($relation->getMorphType()), '=', (new $type())->getMorphClass())->whereHas($belongsTo, $callback, $operator, $count);
                });
            }
    ...
    {
        return $this->whereRelation($relation, $column, $operator, $value)->with([$relation => fn($query) => $column instanceof Closure ? $column($query) : $query->where($column, $operator, $value)]);
    }
    /**
        ...
        return $this->where(function ($query) use($relation, $models) {
            $models->groupBy(fn($model) => $model->getMorphClass())->each(function ($models) use($query, $relation) {
                $query->orWhere(function ($query) use($relation, $models) {
                    $query->where($relation->qualifyColumn($relation->getMorphType()), $models->first()->getMorphClass())->whereIn($relation->qualifyColumn($relation->getForeignKeyName()), $models->map->getKey());
                });
            });
        }, null, null, $boolean);
    }
    ...
    public function whereNotMorphedTo($relation, $model, $boolean = 'and')
    {
        ...
        if (\is_string($model)) {
            ...
            return $this->whereNot(fn($query) => $query->whereNullSafeEquals($relation->qualifyColumn($relation->getMorphType()), $model), null, null, $boolean);
        }
        $models = BaseCollection::wrap($model);
        if ($models->isEmpty()) {
        ...
        return $this->whereNot(function ($query) use($relation, $models) {
            $models->groupBy(fn($model) => $model->getMorphClass())->each(function ($models) use($query, $relation) {
                $query->orWhere(function ($query) use($relation, $models) {
                    $query->whereNullSafeEquals($relation->qualifyColumn($relation->getMorphType()), $models->first()->getMorphClass())->whereIn($relation->qualifyColumn($relation->getForeignKeyName()), $models->map->getKey());
                });
            });
        }, null, null, $boolean);
    }
    ...
    public function whereBelongsTo($related, $relationshipName = null, $boolean = 'and')
    {
        ...
        $this->whereIn($relationship->getQualifiedForeignKeyName(), $relatedCollection->pluck($relationship->getOwnerKeyName())->toArray(), $boolean);
        return $this;
    }
    /**
    ...
    public function whereAttachedTo($related, $relationshipName = null, $boolean = 'and')
    {
        ...
        $this->has($relationshipName, boolean: $boolean, callback: fn(Builder $query) => $query->whereKey($relatedCollection->pluck($related->getKeyName())));
        return $this;
    }
    /**
        ...
        foreach ($this->parseWithRelations($relations) as $name => $constraints) {
            ...
            if ($function) {
                if ($this->getQuery()->getGrammar()->isExpression($column)) {
                    ...
                    $aggregateColumn = $this->getQuery()->getGrammar()->wrap($column === '*' ? $column : $relation->getRelated()->qualifyColumn($hashedColumn));
                }
            ...
            $query = $relation->getRelationExistenceQuery($relation->getRelated()->newQuery(), $this, new Expression($expression))->setBindings([], 'select');
            ...
            $alias ??= Str::snake(\preg_replace('/[^[:alnum:][:space:]_]/u', '', \sprintf('%s %s %s', $name, $function, \strtolower($this->getQuery()->getGrammar()->getValue($column)))));
            if ($function === 'exists') {
    ...
    protected function getRelationHashedColumn($column, $relation)
    {
        ...
        return $this->getQuery()->from === $relation->getQuery()->getQuery()->from ? "{$relation->getRelationCountHash(\false)}.{$column}" : $column;
    }
    /**
    ...
    public function mergeConstraintsFrom(Builder $from)
    {
        ...
        $wheres = $from->getQuery()->from !== $this->getQuery()->from ? $this->requalifyWhereTables($from->getQuery()->wheres, $from->getQuery()->grammar->getValue($from->getQuery()->from), $this->getModel()->getTable()) : $from->getQuery()->wheres;
        // Here we have some other query that we want to merge the where constraints from. We will
        // copy over any where constraints on the query as well as remove any global scopes the
        // query might have removed. Then we will return ourselves with the finished merging.
        return $this->withoutGlobalScopes($from->removedScopes())->mergeWheres($wheres, $whereBindings);
    }
    /**
    ...
    protected function requalifyWhereTables(array $wheres, string $from, string $to) : array
    {
        return (new BaseCollection($wheres))->map(function ($where) use($from, $to) {
            return (new BaseCollection($where))->map(function ($value) use($from, $to) {
                return \is_string($value) && \str_starts_with($value, $from . '.') ? $to . '.' . Str::afterLast($value, '.') : $value;
            });
        })->toArray();
    }
    /**
