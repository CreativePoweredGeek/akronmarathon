...
use BoldMinded\DataGrab\Dependency\Illuminate\Cache\DynamoDbStore;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Cache\Factory as Cache;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Cache\LockProvider;
...
class CacheCommandMutex implements CommandMutex
{
    ...
    public function create($command)
    {
        ...
        $expiresAt = \method_exists($command, 'isolationLockExpiresAt') ? $command->isolationLockExpiresAt() : CarbonInterval::hour();
        ...
        return $store->add($this->commandMutexName($command), \true, $expiresAt);
    }
    /**
    ...
    protected function commandMutexName($command)
    {
        ...
        return \method_exists($command, 'isolatableId') ? $baseName . '-' . $command->isolatableId() : $baseName;
    }
    /**
