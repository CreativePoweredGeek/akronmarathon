...
#[\Symfony\Component\Console\Attribute\AsCommand(name: 'schedule:list')]
class ScheduleListCommand extends Command
{
    ...
    private function listEvent($event, $terminalWidth, $expressionSpacing, $repeatExpressionSpacing, $timezone)
    {
        ...
        $nextDueDate = $this->getNextDueDateForEvent($event, $timezone);
        $nextDueDate = $this->output->isVerbose() ? $nextDueDate->format('Y-m-d H:i:s P') : $nextDueDate->diffForHumans();
        $hasMutex = $event->mutex->exists($event) ? 'Has Mutex › ' : '';
        $dots = \str_repeat('.', \max($terminalWidth - \mb_strwidth($expression . $repeatExpression . $command . $nextDueDateLabel . $nextDueDate . $hasMutex) - 8, 0));
        // Highlight the parameters...
        $command = \preg_replace("#(php artisan [\\w\\-:]+) (.+)#", '$1 <fg=yellow;options=bold>$2</>', $command);
        return [\sprintf('  <fg=yellow>%s</> <fg=#6C7280>%s</> %s<fg=#6C7280>%s %s%s %s</>', $expression, $repeatExpression, $command, $dots, $hasMutex, $nextDueDateLabel, $nextDueDate), $this->output->isVerbose() && \mb_strlen($description) > 1 ? \sprintf('  <fg=#6C7280>%s%s %s</>', \str_repeat(' ', \mb_strlen($expression) + 2), '⇁', $description) : ''];
    }
    /**
     ...
     */
    private function sortEvents(\BoldMinded\DataGrab\Dependency\Illuminate\Support\Collection $events, DateTimeZone $timezone)
    {
    ...
    {
        $nextDueDate = Carbon::instance((new CronExpression($event->expression))->getNextRunDate(Carbon::now()->setTimezone($event->timezone))->setTimezone($timezone));
        if (!$event->isRepeatable()) {
            return $nextDueDate;
        }
        $previousDueDate = Carbon::instance((new CronExpression($event->expression))->getPreviousRunDate(Carbon::now()->setTimezone($event->timezone), allowCurrentDate: \true)->setTimezone($timezone));
    ...
    private function formatCronExpression($expression, $spacing)
    {
        ...
        return (new Collection($spacing))->map(fn($length, $index) => \str_pad($expressions[$index], $length))->implode(' ');
    }
    /**
    ...
    private function getClosureLocation(CallbackEvent $event)
    {
        ...
        if ($callback instanceof Closure) {
            ...
            return \sprintf('%s:%s', \str_replace($this->laravel->basePath() . \DIRECTORY_SEPARATOR, '', $function->getFileName() ?: ''), $function->getStartLine());
        }
