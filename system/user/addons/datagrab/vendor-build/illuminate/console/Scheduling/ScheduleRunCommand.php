...
use BoldMinded\DataGrab\Dependency\Illuminate\Console\Events\ScheduledTaskFailed;
use BoldMinded\DataGrab\Dependency\Illuminate\Console\Events\ScheduledTaskFinished;
use BoldMinded\DataGrab\Dependency\Illuminate\Console\Events\ScheduledTaskSkipped;
use BoldMinded\DataGrab\Dependency\Illuminate\Console\Events\ScheduledTaskStarting;
...
use Throwable;
#[\Symfony\Component\Console\Attribute\AsCommand(name: 'schedule:run')]
class ScheduleRunCommand extends Command
{
    ...
    protected function runSingleServerEvent($event)
    {
        if ($this->schedule->serverShouldRun($event, $this->startedAt)) {
            ...
            $this->components->info(\sprintf('Skipping [%s] because the command already ran on another server.', $event->getSummaryForDisplay()));
        }
    }
    /**
     * Run the given event.
    ...
    protected function runEvent($event)
    {
        ...
        $command = $event instanceof CallbackEvent ? $summary : \trim(\str_replace($this->phpBinary, '', $event->command));
