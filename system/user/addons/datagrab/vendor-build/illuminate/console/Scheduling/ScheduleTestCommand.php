...
use function BoldMinded\DataGrab\Dependency\Laravel\Prompts\select;
#[\Symfony\Component\Console\Attribute\AsCommand(name: 'schedule:test')]
class ScheduleTestCommand extends Command
{
    ...
    public function handle(Schedule $schedule)
    {
        ...
        if (!empty($name = $this->option('name'))) {
            ...
            $matches = \array_filter($commandNames, function ($commandName) use($commandBinary, $name) {
                return \trim(\str_replace($commandBinary, '', $commandName)) === $name;
            });
            if (\count($matches) !== 1) {
        ...
        $command = $event instanceof CallbackEvent ? $summary : \trim(\str_replace($phpBinary, '', $event->command));
    ...
    protected function getSelectedCommandByIndex(array $commandNames)
    {
        if (\count($commandNames) !== \count(\array_unique($commandNames))) {
            ...
            return \array_search(select('Which command would you like to run?', $commandNames), $commandNames);
        }
    }
}
