...
use Closure;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Console\PromptsForMissingInput as PromptsForMissingInputContract;
...
use function BoldMinded\DataGrab\Dependency\Laravel\Prompts\text;
trait PromptsForMissingInput
{
    ...
    {
        $prompted = (new Collection($this->getDefinition()->getArguments()))->reject(fn(InputArgument $argument) => $argument->getName() === 'command')->filter(fn(InputArgument $argument) => $argument->isRequired() && match (\true) {
            ...
            $label = $this->promptForMissingArgumentsUsing()[$argument->getName()] ?? 'What is ' . \lcfirst($argument->getDescription() ?: 'the ' . $argument->getName()) . '?';
            ...
            $answer = text(label: $label, placeholder: $placeholder ?? '', validate: fn($value) => empty($value) ? "The {$argument->getName()} is required." : null);
    ...
    {
        return (new Collection($this->getDefinition()->getOptions()))->reject(fn($option) => $input->getOption($option->getName()) === $option->getDefault())->isNotEmpty();
    }
}
