...
namespace BoldMinded\DataGrab\Dependency\Illuminate\Bus;

use BoldMinded\DataGrab\Dependency\Aws\DynamoDb\DynamoDbClient;
use BoldMinded\DataGrab\Dependency\Aws\DynamoDb\Marshaler;
...
class DynamoBatchRepository implements BatchRepository
{
    ...
    public function get($limit = 50, $before = null)
    {
        ...
        $result = $this->dynamoDbClient->query(['TableName' => $this->table, 'KeyConditionExpression' => $condition, 'ExpressionAttributeValues' => \array_filter([':application' => ['S' => $this->applicationName], ':id' => \array_filter(['S' => $before])]), 'Limit' => $limit, 'ScanIndexForward' => \false]);
        return \array_map(fn($b) => $this->toBatch($this->marshaler->unmarshalItem($b, mapAsObject: \true)), $result['Items']);
    }
    /**
    ...
    public function find(string $batchId)
    {
        ...
        $b = $this->dynamoDbClient->getItem(['TableName' => $this->table, 'Key' => ['application' => ['S' => $this->applicationName], 'id' => ['S' => $batchId]]]);
        if (!isset($b['Item'])) {
            // If we didn't find it via a standard read, attempt consistent read...
            $b = $this->dynamoDbClient->getItem(['TableName' => $this->table, 'Key' => ['application' => ['S' => $this->applicationName], 'id' => ['S' => $batchId]], 'ConsistentRead' => \true]);
            if (!isset($b['Item'])) {
                return null;
            }
        }
        $batch = $this->marshaler->unmarshalItem($b['Item'], mapAsObject: \true);
        if ($batch) {
    ...
    public function store(PendingBatch $batch)
    {
        ...
        $batch = ['id' => $id, 'name' => $batch->name, 'total_jobs' => 0, 'pending_jobs' => 0, 'failed_jobs' => 0, 'failed_job_ids' => [], 'options' => $this->serialize($batch->options ?? []), 'created_at' => \time(), 'cancelled_at' => null, 'finished_at' => null];
        if (!\is_null($this->ttl)) {
            $batch[$this->ttlAttribute] = \time() + $this->ttl;
        }
        $this->dynamoDbClient->putItem(['TableName' => $this->table, 'Item' => $this->marshaler->marshalItem(\array_merge(['application' => $this->applicationName], $batch))]);
        return $this->find($id);
    }
    /**
    ...
    public function incrementTotalJobs(string $batchId, int $amount)
    {
        ...
        $this->dynamoDbClient->updateItem(\array_filter(['TableName' => $this->table, 'Key' => ['application' => ['S' => $this->applicationName], 'id' => ['S' => $batchId]], 'UpdateExpression' => $update, 'ExpressionAttributeValues' => \array_filter([':val' => ['N' => "{$amount}"], ':ttl' => \array_filter(['N' => $this->getExpiryTime()])]), 'ExpressionAttributeNames' => $this->ttlExpressionAttributeName(), 'ReturnValues' => 'ALL_NEW']));
    }
    /**
    ...
    public function markAsFinished(string $batchId)
    {
        ...
        $this->dynamoDbClient->updateItem(\array_filter(['TableName' => $this->table, 'Key' => ['application' => ['S' => $this->applicationName], 'id' => ['S' => $batchId]], 'UpdateExpression' => $update, 'ExpressionAttributeValues' => \array_filter([':timestamp' => ['N' => (string) \time()], ':ttl' => \array_filter(['N' => $this->getExpiryTime()])]), 'ExpressionAttributeNames' => $this->ttlExpressionAttributeName()]));
    }
    /**
    ...
    {
        $this->dynamoDbClient->deleteItem(['TableName' => $this->table, 'Key' => ['application' => ['S' => $this->applicationName], 'id' => ['S' => $batchId]]]);
    }
    /**
    ...
    public function createAwsDynamoTable() : void
    {
        ...
        if (!\is_null($this->ttl)) {
            $this->dynamoDbClient->updateTimeToLive(['TableName' => $this->table, 'TimeToLiveSpecification' => ['AttributeName' => $this->ttlAttribute, 'Enabled' => \true]]);
        }
    }
    /**
    ...
    {
        $this->dynamoDbClient->deleteTable(['TableName' => $this->table]);
    }
    /**
    ...
    {
        return \is_null($this->ttl) ? null : (string) (\time() + $this->ttl);
    }
    /**
