...
use Closure;
use BoldMinded\DataGrab\Dependency\Illuminate\Bus\Events\BatchDispatched;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Container\Container;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Events\Dispatcher as EventDispatcher;
...
class PendingBatch
{
    ...
    protected function ensureJobIsBatchable(object|array $job) : void
    {
        foreach (Arr::wrap($job) as $job) {
            ...
            static::$batchableClasses[$job::class] = \true;
        }
    }
    /**
    ...
    {
        $this->options['before'][] = $callback instanceof Closure ? new SerializableClosure($callback) : $callback;
        return $this;
    }
    /**
    ...
    {
        $this->options['progress'][] = $callback instanceof Closure ? new SerializableClosure($callback) : $callback;
        return $this;
    }
    /**
    ...
    protected function store($repository)
    {
        ...
        (new Collection($this->beforeCallbacks()))->each(function ($handler) use($batch) {
            try {
                ...
                if (\function_exists('BoldMinded\\DataGrab\\Dependency\\report')) {
                    report($e);
                }
            }
        });
