...
namespace BoldMinded\DataGrab\Dependency\Illuminate\View\Compilers\Concerns;
...
use BoldMinded\DataGrab\Dependency\Illuminate\View\ComponentAttributeBag;
trait CompilesComponents
{
    ...
    {
        [$component, $alias, $data] = \str_contains($expression, ',') ? \array_map(\trim(...), \explode(',', \trim($expression, '()'), 3)) + ['', '', ''] : [\trim($expression, '()'), '', ''];
        $component = \trim($component, '\'"');
        $hash = static::newComponentHash($component === AnonymousComponent::class ? $component . ':' . \trim($alias, '\'"') : $component);
    ...
    public function compileEndComponentClass()
    {
        ...
        return $this->compileEndComponent() . "\n" . \implode("\n", ['<?php endif; ?>', '<?php if (isset($__attributesOriginal' . $hash . ')): ?>', '<?php $attributes = $__attributesOriginal' . $hash . '; ?>', '<?php unset($__attributesOriginal' . $hash . '); ?>', '<?php endif; ?>', '<?php if (isset($__componentOriginal' . $hash . ')): ?>', '<?php $component = $__componentOriginal' . $hash . '; ?>', '<?php unset($__componentOriginal' . $hash . '); ?>', '<?php endif; ?>']);
    }
    /**
    ...
    public static function sanitizeComponentAttribute($value)
    {
        ...
        return \is_string($value) || \is_object($value) && !$value instanceof ComponentAttributeBag && \method_exists($value, '__toString') ? e($value) : $value;
    }
}
