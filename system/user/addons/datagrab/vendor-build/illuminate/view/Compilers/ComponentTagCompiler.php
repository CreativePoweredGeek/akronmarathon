...
namespace BoldMinded\DataGrab\Dependency\Illuminate\View\Compilers;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\View\Factory;
...
use BoldMinded\DataGrab\Dependency\Illuminate\View\AnonymousComponent;
use BoldMinded\DataGrab\Dependency\Illuminate\View\DynamicComponent;
use BoldMinded\DataGrab\Dependency\Illuminate\View\ViewFinderInterface;
...
<?php $component->withAttributes([' . $this->attributesToString($attributes->all(), $escapeAttributes = $class !== DynamicComponent::class) . ']); ?>';
    ...
    {
        return (new Collection($this->blade->getAnonymousComponentNamespaces()))->filter(function ($directory, $prefix) use($component) {
    ...
    public function partitionDataAndAttributes($class, array $attributes)
    {
        ...
        $parameterNames = $constructor ? (new Collection($constructor->getParameters()))->map->getName()->all() : [];
        return (new Collection($attributes))->partition(fn($value, $key) => \in_array(Str::camel($key), $parameterNames))->all();
    }
    /**
    ...
    public function compileSlots(string $value)
    {
        ...
        $value = \preg_replace_callback($pattern, function ($matches) {
            ...
            if (!empty($matches['inlineName']) && (!empty($matches['name']) || !empty($matches['boundName']))) {
                $attributes = !empty($matches['name']) ? \array_merge($attributes, $this->getAttributesFromAttributeString('name=' . $matches['name'])) : \array_merge($attributes, $this->getAttributesFromAttributeString(':name=' . $matches['boundName']));
            }
    ...
    {
        return (new Collection(\token_get_all($value)))->map(function ($token) {
            if (!\is_array($token)) {
                return $token;
            }
    ...
    {
        return (new Collection($attributes))->map(function (string $value, string $attribute) use($escapeBound) {
