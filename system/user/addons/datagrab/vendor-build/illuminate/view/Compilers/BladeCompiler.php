...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Support\Htmlable;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\View\Factory as ViewFactory;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\View\View;
...
use BoldMinded\DataGrab\Dependency\Illuminate\View\Component;
...
class BladeCompiler extends Compiler implements CompilerInterface
{
    ...
    public function component($class, $alias = null, $prefix = '')
    {
        ...
        if (\is_null($alias)) {
            $alias = \str_contains($class, '\\View\\Components\\') ? (new Collection(\explode('\\', Str::after($class, '\\View\\Components\\'))))->map(fn($segment) => Str::kebab($segment))->implode(':') : Str::kebab(class_basename($class));
        }
        if (!empty($prefix)) {
            $alias = $prefix . '-' . $alias;
        }
    ...
    {
        $prefix ??= $directory;
        $this->anonymousComponentNamespaces[$prefix] = (new Stringable($directory))->replace('/', '.')->trim('. ')->toString();
    }
    /**
    ...
    {
        $this->directive($name, $handler, bind: \true);
    }
    /**
