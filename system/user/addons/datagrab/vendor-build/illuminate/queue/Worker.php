...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Cache\Repository as CacheContract;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\Factory as QueueManager;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\DetectsLostConnections;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Events\JobAttempted;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Events\JobExceptionOccurred;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Events\JobPopped;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Events\JobPopping;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Events\JobReleasedAfterException;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Events\JobTimedOut;
...
class Worker
{
    ...
    protected function registerTimeoutHandler($job, WorkerOptions $options)
    {
        ...
        \pcntl_signal(\SIGALRM, function () use($job, $options) {
            if ($job) {
                $this->markJobAsFailedIfWillExceedMaxAttempts($job->getConnectionName(), $job, (int) $options->maxTries, $e = $this->timeoutExceededException($job));
    ...
    public function process($connectionName, $job, WorkerOptions $options)
    {
        try {
            ...
            $exceptionOccurred = \true;
            $this->handleJobException($connectionName, $job, $options, $e);
        } finally {
            $this->events->dispatch(new JobAttempted($connectionName, $job, $exceptionOccurred ?? \false));
        }
    }
    /**
    ...
    protected function handleJobException($connectionName, $job, WorkerOptions $options, Throwable $e)
    {
        try {
            ...
            if (!$job->isDeleted() && !$job->isReleased() && !$job->hasFailed()) {
                ...
                $this->events->dispatch(new JobReleasedAfterException($connectionName, $job));
            }
        }
        throw $e;
    }
    ...
    protected function listenForSignals()
    {
        ...
        \pcntl_signal(\SIGQUIT, fn() => $this->shouldQuit = \true);
        \pcntl_signal(\SIGTERM, fn() => $this->shouldQuit = \true);
        \pcntl_signal(\SIGUSR2, fn() => $this->paused = \true);
