...
use Exception;
use BoldMinded\DataGrab\Dependency\Illuminate\Bus\Batchable;
use BoldMinded\DataGrab\Dependency\Illuminate\Bus\UniqueLock;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Bus\Dispatcher;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Cache\Factory as CacheFactory;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\ShouldBeUniqueUntilProcessing;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Eloquent\ModelNotFoundException;
use BoldMinded\DataGrab\Dependency\Illuminate\Log\Context\Repository as ContextRepository;
use BoldMinded\DataGrab\Dependency\Illuminate\Pipeline\Pipeline;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Attributes\DeleteWhenMissingModels;
use ReflectionClass;
...
class CallQueuedHandler
{
    ...
    protected function handleModelNotFound(Job $job, $e)
    {
        ...
        try {
            ...
            $shouldDelete = $reflectionClass->getDefaultProperties()['deleteWhenMissingModels'] ?? \count($reflectionClass->getAttributes(DeleteWhenMissingModels::class)) !== 0;
        } catch (Exception) {
            $shouldDelete = \false;
        }
    ...
    {
        if (!$this->container->bound(ContextRepository::class) || !$this->container->bound(CacheFactory::class)) {
            return;
        }
        $context = $this->container->make(ContextRepository::class);
        [$store, $key] = [$context->getHidden('laravel_unique_job_cache_store'), $context->getHidden('laravel_unique_job_key')];
        if ($store && $key) {
