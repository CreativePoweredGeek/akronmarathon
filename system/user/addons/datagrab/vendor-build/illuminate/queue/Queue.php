...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Encryption\Encrypter;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\ShouldBeEncrypted;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\ShouldQueueAfterCommit;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Events\JobQueued;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Events\JobQueueing;
...
abstract class Queue
{
    ...
    {
        $payload = $this->withCreatePayloadHooks($queue, ['uuid' => (string) Str::uuid(), 'displayName' => $this->getDisplayName($job), 'job' => 'Illuminate\\Queue\\CallQueuedHandler@call', 'maxTries' => $this->getJobTries($job), 'maxExceptions' => $job->maxExceptions ?? null, 'failOnTimeout' => $job->failOnTimeout ?? \false, 'backoff' => $this->getJobBackoff($job), 'timeout' => $job->timeout ?? null, 'retryUntil' => $this->getJobExpiration($job), 'data' => ['commandName' => $job, 'command' => $job]]);
        ...
        return \array_merge($payload, ['data' => \array_merge($payload['data'], ['commandName' => \get_class($job), 'command' => $command])]);
    }
    /**
    ...
    public function getJobBackoff($job)
    {
        ...
        return Collection::wrap($backoff)->map(fn($backoff) => $backoff instanceof DateTimeInterface ? $this->secondsUntil($backoff) : $backoff)->implode(',');
    }
    /**
    ...
    {
        return $this->withCreatePayloadHooks($queue, ['uuid' => (string) Str::uuid(), 'displayName' => \is_string($job) ? \explode('@', $job)[0] : null, 'job' => $job, 'maxTries' => null, 'maxExceptions' => null, 'failOnTimeout' => \false, 'backoff' => null, 'timeout' => null, 'data' => $data]);
    }
    /**
    ...
    protected function enqueueUsing($job, $payload, $queue, $delay, $callback)
    {
        if ($this->shouldDispatchAfterCommit($job) && $this->container->bound('db.transactions')) {
            return $this->container->make('db.transactions')->addCallback(function () use($queue, $job, $payload, $delay, $callback) {
