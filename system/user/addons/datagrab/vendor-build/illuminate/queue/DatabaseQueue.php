...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\ClearableQueue;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\Queue as QueueContract;
use BoldMinded\DataGrab\Dependency\Illuminate\Database\Connection;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Jobs\DatabaseJob;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Jobs\DatabaseJobRecord;
...
class DatabaseQueue extends Queue implements QueueContract, ClearableQueue
{
     ...
     * @return void
     */
    public function __construct(Connection $database, $table, $default = 'default', $retryAfter = 60, $dispatchAfterCommit = \false)
    {
        $this->table = $table;
    ...
    {
        return $this->enqueueUsing($job, $this->createPayload($job, $this->getQueue($queue), $data), $queue, null, function ($payload, $queue) {
    ...
    public function bulk($jobs, $data = '', $queue = null)
    {
        ...
        return $this->database->table($this->table)->insert((new Collection((array) $jobs))->map(function ($job) use($queue, $data, $now) {
            return $this->buildDatabaseRecord($queue, $this->createPayload($job, $this->getQueue($queue), $data), isset($job->delay) ? $this->availableAt($job->delay) : $now);
        })->all());
    }
    /**
    ...
    {
        $job = $this->database->table($this->table)->lock($this->getLockForPopping())->where('queue', $this->getQueue($queue))->where(function ($query) {
    ...
    protected function getLockForPopping()
    {
        ...
        if ($databaseEngine === 'mysql' && \version_compare($databaseVersion, '8.0.1', '>=') || $databaseEngine === 'mariadb' && \version_compare($databaseVersion, '10.6.0', '>=') || $databaseEngine === 'pgsql' && \version_compare($databaseVersion, '9.5', '>=') || $databaseEngine === 'vitess' && \version_compare($databaseVersion, '19.0', '>=')) {
    ...
    protected function isAvailable($query)
    {
        $query->where(function ($query) {
            $query->whereNull('reserved_at')->where('available_at', '<=', $this->currentTime());
        });
    }
    /**
    ...
    {
        $this->database->table($this->table)->where('id', $job->id)->update(['reserved_at' => $job->touch(), 'attempts' => $job->increment()]);
        return $job;
    }
    /**
