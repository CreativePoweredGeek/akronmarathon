...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\Factory;
...
#[\Symfony\Component\Console\Attribute\AsCommand(name: 'queue:monitor')]
class MonitorCommand extends Command
{
    ...
    protected function parseQueues($queues)
    {
        return (new Collection(\explode(',', $queues)))->map(function ($queue) {
            ...
            return ['connection' => $connection, 'queue' => $queue, 'size' => $size = $this->manager->connection($connection)->size($queue), 'status' => $size >= $this->option('max') ? '<fg=yellow;options=bold>ALERT</>' : '<fg=green;options=bold>OK</>'];
        });
    }
    /**
