...
#[\Symfony\Component\Console\Attribute\AsCommand(name: 'queue:retry')]
class RetryCommand extends Command
{
    ...
    protected function getJobIds()
    {
        ...
        if (\count($ids) === 1 && $ids[0] === 'all') {
            ...
            return \method_exists($failer, 'ids') ? $failer->ids() : Arr::pluck($failer->all(), 'id');
        }
    ...
    {
        $failer = $this->laravel['queue.failer'];
        $ids = \method_exists($failer, 'ids') ? $failer->ids($queue) : (new Collection($failer->all()))->where('queue', $queue)->pluck('id')->toArray();
        if (\count($ids) === 0) {
    ...
    {
        $this->laravel['queue']->connection($job->connection)->pushRaw($this->refreshRetryUntil($this->resetAttempts($job->payload)), $job->queue);
    }
    /**
