...
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Jobs\BeanstalkdJob;
use BoldMinded\DataGrab\Dependency\Pheanstalk\Contract\JobIdInterface;
use BoldMinded\DataGrab\Dependency\Pheanstalk\Pheanstalk;
use BoldMinded\DataGrab\Dependency\Pheanstalk\Values\Job;
use BoldMinded\DataGrab\Dependency\Pheanstalk\Values\JobId;
use BoldMinded\DataGrab\Dependency\Pheanstalk\Values\TubeName;
class BeanstalkdQueue extends Queue implements QueueContract
{
     ...
     * @return void
     */
    public function __construct($pheanstalk, $default, $timeToRun, $blockFor = 0, $dispatchAfterCommit = \false)
    {
    ...
    public function pushRaw($payload, $queue = null, array $options = [])
    {
        ...
        return $this->pheanstalk->put($payload, Pheanstalk::DEFAULT_PRIORITY, Pheanstalk::DEFAULT_DELAY, $this->timeToRun);
    }
    /**
    ...
    public function later($delay, $job, $data = '', $queue = null)
    {
        return $this->enqueueUsing($job, $this->createPayload($job, $this->getQueue($queue), $data), $queue, $delay, function ($payload, $queue, $delay) {
            $this->pheanstalk->useTube(new TubeName($this->getQueue($queue)));
            return $this->pheanstalk->put($payload, Pheanstalk::DEFAULT_PRIORITY, $this->secondsUntil($delay), $this->timeToRun);
        });
    }
    /**
    ...
    {
        $this->pheanstalk->watch($tube = new TubeName($queue = $this->getQueue($queue)));
