...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\Job as JobContract;
use BoldMinded\DataGrab\Dependency\Illuminate\Queue\Jobs\FakeJob;
...
trait InteractsWithQueue
{
    ...
    public function assertDeleted()
    {
        ...
        PHPUnit::assertTrue($this->job->isDeleted(), 'Job was expected to be deleted, but was not.');
        return $this;
    }
    /**
    ...
    {
        $this->ensureQueueInteractionsHaveBeenFaked();
        PHPUnit::assertTrue(!$this->job->isDeleted(), 'Job was unexpectedly deleted.');
        return $this;
    }
    /**
    ...
    {
        $this->ensureQueueInteractionsHaveBeenFaked();
        PHPUnit::assertTrue($this->job->hasFailed(), 'Job was expected to be manually failed, but was not.');
        return $this;
    }
    /**
    ...
    public function assertFailedWith($exception)
    {
        ...
        if (\is_string($exception) && \class_exists($exception)) {
            PHPUnit::assertInstanceOf($exception, $this->job->failedWith, 'Expected job to be manually failed with [' . $exception . '] but job failed with [' . \get_class($this->job->failedWith) . '].');
            return $this;
        }
        if (\is_string($exception)) {
            $exception = new ManuallyFailedException($exception);
        ...
        if ($exception instanceof Throwable) {
            ...
            PHPUnit::assertEquals($exception->getCode(), $this->job->failedWith->getCode(), 'Expected exception code [' . $exception->getCode() . '] but job failed with exception code [' . $this->job->failedWith->getCode() . '].');
            PHPUnit::assertEquals($exception->getMessage(), $this->job->failedWith->getMessage(), 'Expected exceptoin message [' . $exception->getMessage() . '] but job failed with exception message [' . $this->job->failedWith->getMessage() . '].');
        }
        return $this;
    }
    /**
    ...
    {
        $this->ensureQueueInteractionsHaveBeenFaked();
        PHPUnit::assertTrue(!$this->job->hasFailed(), 'Job was unexpectedly failed manually.');
        return $this;
    }
    /**
    ...
    public function assertReleased($delay = null)
    {
        $this->ensureQueueInteractionsHaveBeenFaked();
        $delay = $delay instanceof DateTimeInterface ? $this->secondsUntil($delay) : $delay;
        PHPUnit::assertTrue($this->job->isReleased(), 'Job was expected to be released, but was not.');
        if (!\is_null($delay)) {
            PHPUnit::assertSame($delay, $this->job->releaseDelay, "Expected job to be released with delay of [{$delay}] seconds, but was released with delay of [{$this->job->releaseDelay}] seconds.");
        }
        return $this;
    }
    /**
    ...
    {
        $this->ensureQueueInteractionsHaveBeenFaked();
        PHPUnit::assertTrue(!$this->job->isReleased(), 'Job was unexpectedly released.');
        return $this;
    }
    /**
