...
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Traits\EnumeratesValues;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Traits\Macroable;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Traits\TransformsToResourceCollection;
...
class Collection implements ArrayAccess, CanBeEscapedWhenCastToString, Enumerable
{
    ...
    {
        $values = (isset($key) ? $this->pluck($key) : $this)->reject(fn($item) => \is_null($item))->sort()->values();
        $count = $values->count();
        if ($count === 0) {
            return;
        }
     ...
     */
    public function chunk($size, $preserveKeys = \true)
    {
        if ($size <= 0) {
            return new static();
        }
