...
namespace BoldMinded\DataGrab\Dependency\Illuminate\Filesystem;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Filesystem\Cloud as CloudFilesystemContract;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Filesystem\Filesystem as FilesystemContract;
use BoldMinded\DataGrab\Dependency\Illuminate\Http\File;
use BoldMinded\DataGrab\Dependency\Illuminate\Http\Request;
use BoldMinded\DataGrab\Dependency\Illuminate\Http\UploadedFile;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Arr;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Str;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Traits\Conditionable;
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Traits\Macroable;
use InvalidArgumentException;
use BoldMinded\DataGrab\Dependency\League\Flysystem\FilesystemAdapter as FlysystemAdapter;
use BoldMinded\DataGrab\Dependency\League\Flysystem\FilesystemOperator;
use BoldMinded\DataGrab\Dependency\League\Flysystem\Ftp\FtpAdapter;
use BoldMinded\DataGrab\Dependency\League\Flysystem\Local\LocalFilesystemAdapter as LocalAdapter;
use BoldMinded\DataGrab\Dependency\League\Flysystem\PathPrefixer;
use BoldMinded\DataGrab\Dependency\League\Flysystem\PhpseclibV3\SftpAdapter;
use BoldMinded\DataGrab\Dependency\League\Flysystem\StorageAttributes;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToCopyFile;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToCreateDirectory;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToDeleteDirectory;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToDeleteFile;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToMoveFile;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToProvideChecksum;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToReadFile;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToRetrieveMetadata;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToSetVisibility;
use BoldMinded\DataGrab\Dependency\League\Flysystem\UnableToWriteFile;
use BoldMinded\DataGrab\Dependency\League\Flysystem\Visibility;
use PHPUnit\Framework\Assert as PHPUnit;
use BoldMinded\DataGrab\Dependency\Psr\Http\Message\StreamInterface;
use RuntimeException;
use BoldMinded\DataGrab\Dependency\Symfony\Component\HttpFoundation\StreamedResponse;
/**
...
class FilesystemAdapter implements CloudFilesystemContract
{
    ...
    public function assertExists($path, $content = null)
    {
        ...
        foreach ($paths as $path) {
            PHPUnit::assertTrue($this->exists($path), "Unable to find a file or directory at path [{$path}].");
            if (!\is_null($content)) {
                $actual = $this->get($path);
                PHPUnit::assertSame($content, $actual, "File or directory [{$path}] was found, but content [{$actual}] does not match [{$content}].");
            }
        }
        return $this;
    }
    ...
    public function assertCount($path, $count, $recursive = \false)
    {
        ...
        PHPUnit::assertEquals($count, $actual, "Expected [{$count}] files at [{$path}], but found [{$actual}].");
        return $this;
    }
    /**
    ...
    public function assertMissing($path)
    {
        ...
        $paths = Arr::wrap($path);
        foreach ($paths as $path) {
            PHPUnit::assertFalse($this->exists($path), "Found unexpected file or directory at path [{$path}].");
        }
        return $this;
    }
    /**
    ...
    {
        PHPUnit::assertEmpty($this->allFiles($path), "Directory [{$path}] is not empty.");
        return $this;
    }
    /**
    ...
    public function json($path, $flags = 0)
    {
        ...
        return \is_null($content) ? null : \json_decode($content, \true, 512, $flags);
    }
    /**
    ...
    {
        return isset($this->serveCallback) ? \call_user_func($this->serveCallback, $request, $path, $headers) : $this->response($path, $name, $headers);
    }
    /**
     * Create a streamed download response for a given file.
     *
    ...
    public function put($path, $contents, $options = [])
    {
        ...
        try {
            ...
            \is_resource($contents) ? $this->driver->writeStream($path, $contents, $options) : $this->driver->write($path, $contents, $options);
    ...
    {
        return isset($this->config['url']) ? $this->concatPathToUrl($this->config['url'], $path) : $path;
    }
    /**
     * Get the URL for the file at the given path.
     *
    ...
    {
        return $this->driver->listContents($directory ?? '', $recursive)->filter(function (StorageAttributes $attributes) {
            return $attributes->isFile();
        })->sortByPath()->map(function (StorageAttributes $attributes) {
