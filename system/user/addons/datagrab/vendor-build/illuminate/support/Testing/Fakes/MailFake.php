...
use Closure;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Mail\Factory;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Mail\Mailable;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Mail\Mailer;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Mail\MailQueue;
use BoldMinded\DataGrab\Dependency\Illuminate\Contracts\Queue\ShouldQueue;
use BoldMinded\DataGrab\Dependency\Illuminate\Mail\MailManager;
...
use BoldMinded\DataGrab\Dependency\Illuminate\Support\Traits\ReflectsClosures;
...
class MailFake implements Factory, Fake, Mailer, MailQueue
{
    ...
    public function assertSent($mailable, $callback = null)
    {
        ...
        if (\is_array($callback) || \is_string($callback)) {
            foreach (Arr::wrap($callback) as $address) {
                ...
                PHPUnit::assertTrue($this->sent($mailable, $callback)->count() > 0, "The expected [{$mailable}] mailable was not sent to address [{$address}]." . $suggestion);
            }
            return;
        }
        PHPUnit::assertTrue($this->sent($mailable, $callback)->count() > 0, "The expected [{$mailable}] mailable was not sent." . $suggestion);
    }
    /**
    ...
    public function assertNotSent($mailable, $callback = null)
    {
        if (\is_string($callback) || \is_array($callback)) {
            foreach (Arr::wrap($callback) as $address) {
                $callback = fn($mail) => $mail->hasTo($address);
                PHPUnit::assertCount(0, $this->sent($mailable, $callback), "The unexpected [{$mailable}] mailable was sent to address [{$address}].");
            }
            return;
        }
        [$mailable, $callback] = $this->prepareMailableAndCallback($mailable, $callback);
    ...
    {
        $mailableNames = (new Collection($this->mailables))->map(fn($mailable) => \get_class($mailable))->join("\n- ");
    ...
    public function assertQueued($mailable, $callback = null)
    {
        ...
        if (\is_string($callback) || \is_array($callback)) {
            foreach (Arr::wrap($callback) as $address) {
                $callback = fn($mail) => $mail->hasTo($address);
                PHPUnit::assertTrue($this->queued($mailable, $callback)->count() > 0, "The expected [{$mailable}] mailable was not queued to address [{$address}].");
            }
            return;
        }
    ...
    protected function assertQueuedTimes($mailable, $times = 1)
    {
        ...
        PHPUnit::assertSame($times, $count, \sprintf("The expected [{$mailable}] mailable was queued {$count} %s instead of {$times} %s.", Str::plural('time', $count), Str::plural('time', $times)));
    }
    /**
    ...
    public function assertNotQueued($mailable, $callback = null)
    {
        if (\is_string($callback) || \is_array($callback)) {
            foreach (Arr::wrap($callback) as $address) {
                $callback = fn($mail) => $mail->hasTo($address);
                PHPUnit::assertCount(0, $this->queued($mailable, $callback), "The unexpected [{$mailable}] mailable was queued to address [{$address}].");
            }
            return;
        }
        [$mailable, $callback] = $this->prepareMailableAndCallback($mailable, $callback);
    ...
    {
        $mailableNames = (new Collection($this->queuedMailables))->map(fn($mailable) => \get_class($mailable))->join("\n- ");
    ...
    public function assertSentCount($count)
    {
        ...
        PHPUnit::assertSame($count, $total, "The total number of mailables sent was {$total} instead of {$count}.");
    }
    /**
    ...
    public function assertQueuedCount($count)
    {
        ...
        PHPUnit::assertSame($count, $total, "The total number of mailables queued was {$total} instead of {$count}.");
    }
    /**
    ...
    {
        $total = (new Collection($this->mailables))->concat($this->queuedMailables)->count();
        PHPUnit::assertSame($count, $total, "The total number of outgoing mailables was {$total} instead of {$count}.");
    }
    /**
