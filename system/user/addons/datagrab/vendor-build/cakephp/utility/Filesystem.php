...
class Filesystem
{
    ...
    public function findRecursive(string $path, Closure|string|null $filter = null, ?int $flags = null) : Iterator
    {
        ...
        $dirFilter = new RecursiveCallbackFilterIterator($directory, function (SplFileInfo $current) {
            if (\str_starts_with($current->getFilename(), '.') && $current->isDir()) {
                return \false;
            }
            return \true;
        });
        $flatten = new RecursiveIteratorIterator($dirFilter, RecursiveIteratorIterator::CHILD_FIRST);
        if ($filter === null) {
            return $flatten;
        }
    ...
    public function copyDir(string $source, string $destination) : bool
    {
        ...
        foreach ($iterator as $fileInfo) {
            if ($fileInfo->isDir()) {
                $result = $result && $this->copyDir($fileInfo->getPathname(), $destination . \DIRECTORY_SEPARATOR . $fileInfo->getFilename());
            } else {
                // phpcs:ignore
                $result = $result && @\copy($fileInfo->getPathname(), $destination . \DIRECTORY_SEPARATOR . $fileInfo->getFilename());
            }
        }
        return $result;
    }
