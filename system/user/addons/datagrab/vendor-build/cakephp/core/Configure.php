...
use BoldMinded\DataGrab\Dependency\Cake\Cache\Cache;
use BoldMinded\DataGrab\Dependency\Cake\Core\Configure\ConfigEngineInterface;
use BoldMinded\DataGrab\Dependency\Cake\Core\Configure\Engine\PhpConfig;
...
class Configure
{
     ...
     */
    protected static array $_values = ['debug' => \false];
    /**
     ...
     */
    public static function load(string $key, string $config = 'default', bool $merge = \true) : bool
    {
        $engine = static::_getEngine($config);
        if (!$engine) {
            throw new CakeException(\sprintf('Config %s engine not found when attempting to load %s.', $config, $key));
        }
