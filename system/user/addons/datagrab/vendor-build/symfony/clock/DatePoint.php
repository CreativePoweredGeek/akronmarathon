 ...
 */
namespace BoldMinded\DataGrab\Dependency\Symfony\Component\Clock;
...
final class DatePoint extends \DateTimeImmutable
{
        ...
        if ('now' !== $datetime) {
            ...
            if (\PHP_VERSION_ID < 80300) {
                try {
                    ...
                    throw new \BoldMinded\DataGrab\Dependency\DateMalformedStringException($e->getMessage(), $e->getCode(), $e);
                }
            } else {
                $builtInDate = new parent($datetime, $timezone ?? $now->getTimezone());
                $timezone = $builtInDate->getTimezone();
    ...
    {
        return parent::createFromFormat($format, $datetime, $timezone) ?: throw new \BoldMinded\DataGrab\Dependency\DateMalformedStringException(static::getLastErrors()['errors'][0] ?? 'Invalid date string or format.');
    }
    ...
    public static function createFromTimestamp(int|float $timestamp) : static
    {
        ...
        if (!\is_finite($timestamp) || \PHP_INT_MAX + 1.0 <= $timestamp || \PHP_INT_MIN > $timestamp) {
            throw new \BoldMinded\DataGrab\Dependency\DateRangeError(\sprintf('DateTimeImmutable::createFromTimestamp(): Argument #1 ($timestamp) must be a finite number between %s and %s.999999, %s given', \PHP_INT_MIN, \PHP_INT_MAX, $timestamp));
        }
        if ($timestamp < 0) {
    ...
    {
        if (\PHP_VERSION_ID < 80300) {
            return @parent::modify($modifier) ?: throw new \BoldMinded\DataGrab\Dependency\DateMalformedStringException(\error_get_last()['message'] ?? \sprintf('Invalid modifier: "%s".', $modifier));
        }
    ...
    {
        return parent::getTimezone() ?: throw new \BoldMinded\DataGrab\Dependency\DateInvalidTimeZoneException('The DatePoint object has no timezone.');
    }
    public function setMicrosecond(int $microsecond) : static
    {
        if ($microsecond < 0 || $microsecond > 999999) {
            throw new \BoldMinded\DataGrab\Dependency\DateRangeError('DatePoint::setMicrosecond(): Argument #1 ($microsecond) must be between 0 and 999999, ' . $microsecond . ' given');
        }
        if (\PHP_VERSION_ID < 80400) {
