...
abstract readonly class BigNumber implements JsonSerializable, Stringable
{
     ...
     * @pure
     */
    public static final function ofNullable(BigNumber|int|float|string|null $value) : ?static
    {
        if (is_null($value)) {
            return null;
        }
     ...
     * @pure
     */
    public final function abs() : static
    {
     ...
     * @pure
     */
    public abstract function negated() : static;
    /**
     ...
     * @pure
     */
    public final function clamp(BigNumber|int|float|string $min, BigNumber|int|float|string $max) : static
    {
        $min = static::of($min);
        $max = static::of($max);
    ...
    private static function _of(BigNumber|int|float|string $value) : BigNumber
    {
        ...
        if (is_float($value)) {
            ...
            trigger_error('Passing floats to BigNumber::of() and arithmetic methods is deprecated and will be removed in 0.15. ' . 'Cast the float to string explicitly to preserve the previous behaviour.', E_USER_DEPRECATED);
            if (is_nan($value)) {
                $value = 'NAN';
            } else {
                $value = (string) $value;
