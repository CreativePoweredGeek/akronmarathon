 ...
 */
namespace BoldMinded\DataGrab\Dependency\Carbon;

use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\BadFluentConstructorException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\BadFluentSetterException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\InvalidCastException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\InvalidFormatException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\InvalidIntervalException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\OutOfRangeException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\ParseErrorException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\UnitNotConfiguredException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\UnknownGetterException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\UnknownSetterException;
use BoldMinded\DataGrab\Dependency\Carbon\Exceptions\UnknownUnitException;
use BoldMinded\DataGrab\Dependency\Carbon\Traits\IntervalRounding;
use BoldMinded\DataGrab\Dependency\Carbon\Traits\IntervalStep;
use BoldMinded\DataGrab\Dependency\Carbon\Traits\LocalFactory;
use BoldMinded\DataGrab\Dependency\Carbon\Traits\MagicParameter;
use BoldMinded\DataGrab\Dependency\Carbon\Traits\Mixin;
use BoldMinded\DataGrab\Dependency\Carbon\Traits\Options;
use BoldMinded\DataGrab\Dependency\Carbon\Traits\ToStringFormat;
use Closure;
use DateInterval;
use DateTime;
...
class CarbonInterval extends DateInterval implements CarbonConverterInterface
{
    ...
    public const SPECIAL_TRANSLATIONS = [1 => ['option' => CarbonInterface::ONE_DAY_WORDS, 'future' => 'diff_tomorrow', 'past' => 'diff_yesterday'], 2 => ['option' => CarbonInterface::TWO_DAY_WORDS, 'future' => 'diff_after_tomorrow', 'past' => 'diff_before_yesterday']];
    ...
    public function setTimezone(DateTimeZone|string|int $timezone) : static
    {
        ...
        if ($this->startDate) {
            $this->startDate = $this->startDate->avoidMutation()->setTimezone($timezone);
            $this->rawInterval = null;
        }
        if ($this->endDate) {
            $this->endDate = $this->endDate->avoidMutation()->setTimezone($timezone);
            $this->rawInterval = null;
        }
        return $this;
    }
    ...
    public function shiftTimezone(DateTimeZone|string|int $timezone) : static
    {
        ...
        $this->checkStartAndEnd();
        if ($this->startDate) {
            $this->startDate = $this->startDate->avoidMutation()->shiftTimezone($timezone);
            $this->rawInterval = null;
        }
        if ($this->endDate) {
            $this->endDate = $this->endDate->avoidMutation()->shiftTimezone($timezone);
            $this->rawInterval = null;
        }
        return $this;
    }
    ...
    public static function fromString(string $intervalDefinition) : static
    {
        ...
        return self::withOriginal(new static($years, $months, $weeks, $days, $hours, $minutes, $seconds, $milliseconds * Carbon::MICROSECONDS_PER_MILLISECOND + $microseconds), $intervalDefinition);
    }
    /**
     ...
     * @return $this
     */
    public function absolute(bool $absolute = \true) : static
    {
    ...
    public static function createFromDateString(string $datetime) : static
    {
        ...
        $interval ?: throw new InvalidFormatException('Could not create interval from: ' . \var_export($datetime, \true), previous: $previousException);
    ...
    protected function getForHumansParameters($syntax = null, $short = \false, $parts = self::NO_LIMIT, $options = null) : array
    {
        ...
        $skip = \array_map(static fn($unit) => $unit instanceof Unit ? $unit->value : $unit, (array) $skip);
        $skip = \array_map('strtolower', \array_filter($skip, static fn($unit) => \is_string($unit) && $unit !== ''));
    ...
    public function forHumans($syntax = null, $short = \false, $parts = self::NO_LIMIT, $options = null) : string
    {
        ...
        [$syntax, $short, $parts, $options, $join, $aUnit, $altNumbers, $interpolations, $minimumUnit, $skip, $translator] = $this->getForHumansParameters($syntax, $short, $parts, $options);
        $interval = [];
    ...
    public function toPeriod(...$params) : CarbonPeriod
    {
        if ($this->timezoneSetting) {
            $timeZone = \is_string($this->timezoneSetting) ? new DateTimeZone($this->timezoneSetting) : $this->timezoneSetting;
    ...
    public function equalTo($interval) : bool
    {
        ...
        return $cascadedInterval->invert === $comparedInterval->invert && $cascadedInterval->getNonZeroValues() === $comparedInterval->getNonZeroValues();
    }
    /**
     ...
     * @return bool
     */
    public function between($interval1, $interval2, bool $equal = \true) : bool
    {
     ...
     * @return bool
     */
    public function isBetween($interval1, $interval2, bool $equal = \true) : bool
    {
    ...
    public function roundUnit(string $unit, DateInterval|string|int|float $precision = 1, string $function = 'round') : static
    {
        ...
        $this->copyProperties($next->roundUnit($unit, $precision, $function)->diff($base));
    ...
    {
        $properties = \array_combine(\array_map(static fn(mixed $key) => \is_string($key) ? \str_replace('tzName', 'timezoneSetting', $key) : $key, \array_keys($data)), $data);
    ...
    {
        if ($this->initialValues !== null && ($this->startDate !== null || $this->endDate !== null) && $this->initialValues !== $this->getInnerValues()) {
            $this->absolute = \false;
            $this->startDate = null;
            $this->endDate = null;
            $this->rawInterval = null;
