...
class Factory
{
     ...
     * Days of weekend.
     */
    protected array $weekendDays = [CarbonInterface::SATURDAY, CarbonInterface::SUNDAY];
    /**
    ...
    public function macro(string $name, ?callable $macro) : void
    {
        ...
        $macros[$name] = $macro;
        $this->mergeSettings(['macros' => $macros]);
    }
    /**
    ...
    public function handleTestNowClosure(Closure|CarbonInterface|null $testNow, DateTimeZone|string|int|null $timezone = null) : ?CarbonInterface
    {
        if ($testNow instanceof Closure) {
            ...
            $testNow = $testNow($callback->prepareParameter($this->parse($realNow->format('Y-m-d H:i:s.u'), $timezone ?? $realNow->getTimezone())));
            if ($testNow !== null && !$testNow instanceof DateTimeInterface) {
                ...
                throw new RuntimeException('The test closure defined in ' . $function->getFileName() . ' at line ' . $function->getStartLine() . ' returned ' . $type . '; expected ' . CarbonInterface::class . '|null');
            }
