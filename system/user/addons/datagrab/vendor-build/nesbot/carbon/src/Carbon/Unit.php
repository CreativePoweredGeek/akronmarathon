...
enum Unit : string
{
                ...
                foreach (self::cases() as $unit) {
                    foreach (['', '_from_now', '_ago', '_after', '_before'] as $suffix) {
                        ...
                        if (\is_string($message)) {
                            $words = \explode('|', \mb_strtolower(\preg_replace('/[{\\[\\]].+?[}\\[\\]]/', '', \str_replace(':count', '', $message))));
                            foreach ($words as $word) {
                                if (\trim($word) === $lowerName) {
                                    return $unit;
                                }
                            }
                        }
    ...
    {
        if ($locale !== null) {
            return \trim(Translator::get($locale)->trans($this->value, ['%count%' => 1, ':count' => 1]), "1 \n\r\t\v\x00");
        }
        return $this->value;
    }
    ...
    {
        if ($locale !== null) {
            return \trim(Translator::get($locale)->trans($this->value, ['%count%' => 9, ':count' => 9]), "9 \n\r\t\v\x00");
        }
