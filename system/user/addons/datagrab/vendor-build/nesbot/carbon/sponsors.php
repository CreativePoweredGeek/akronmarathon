...
function getOpenCollectiveSponsors() : string
{
    ...
    $list = \array_map(static function (array $member) : array {
        ...
        $monthlyContribution = (float) ($isYearly && $lastTransactionAt > CarbonImmutable::parse('-1 year') ? $member['lastTransactionAmount'] / 11.2 : $member['totalAmountDonated'] / \ceil($createdAt->floatDiffInMonths()));
        if (!$isYearly) {
        ...
        $yearlyContribution = (float) ($isYearly ? 12 * $monthlyContribution : $member['totalAmountDonated'] / \max(1, $createdAt->floatDiffInYears()));
        $status = null;
        $rank = 0;
    ...
    foreach ($list as $member) {
        ...
        if (isset($membersByUrl[$url]) || !\in_array($member['status'], ['sponsor', 'backerPlus'], \true)) {
            continue;
        }
        ...
        $output .= "\n" . '<a title="' . $title . '" href="' . $href . '" target="_blank"' . $rel . '>' . '<img alt="' . $alt . '" src="' . $src . '" width="' . $width . '" height="' . $height . '">' . '</a>';
    }
    return $output;
}
