...
namespace BoldMinded\DataGrab\Dependency\Laravel\SerializableClosure\Support;
...
class ReflectionClosure extends ReflectionFunction
{
    ...
    {
        $candidates[] = ['code' => $code, 'use' => $use, 'isShortClosure' => $isShortClosure, 'isUsingThisObject' => $isUsingThisObject, 'isUsingScope' => $isUsingScope];
    ...
    protected function applyCandidate($candidate)
    {
        if ($candidate['isShortClosure']) {
            ...
            $this->useVariables = empty($candidate['use']) ? $candidate['use'] : \array_intersect_key($this->getStaticVariables(), \array_flip($candidate['use']));
        }
