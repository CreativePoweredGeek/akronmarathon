...
class MultiSearchPromptRenderer extends Renderer implements Scrolling
{
    ...
    public function __invoke(MultiSearchPrompt $prompt) : string
    {
        ...
        return match ($prompt->state) {
            ...
            'error' => $this->box($this->truncate($prompt->label, $prompt->terminal()->cols() - 6), $prompt->valueWithCursor($maxWidth), $this->renderOptions($prompt), color: 'yellow', info: $this->getInfoText($prompt))->warning($this->truncate($prompt->error, $prompt->terminal()->cols() - 5)),
            'searching' => $this->box($this->cyan($this->truncate($prompt->label, $prompt->terminal()->cols() - 6)), $this->valueWithCursorAndSearchIcon($prompt, $maxWidth), $this->renderOptions($prompt), info: $this->getInfoText($prompt))->hint($prompt->hint),
            default => $this->box($this->cyan($this->truncate($prompt->label, $prompt->terminal()->cols() - 6)), $prompt->valueWithCursor($maxWidth), $this->renderOptions($prompt), info: $this->getInfoText($prompt))->when($prompt->hint, fn() => $this->hint($prompt->hint), fn() => $this->newLine())->spaceForDropdown($prompt),
        };
    }
    /**
    ...
    protected function spaceForDropdown(MultiSearchPrompt $prompt) : self
    {
        ...
        $this->newLine(\max(0, \min($prompt->scroll, $prompt->terminal()->lines() - 7) - \count($prompt->matches())));
        if ($prompt->matches() === []) {
            $this->newLine();
        }
        return $this;
    ...
    protected function renderOptions(MultiSearchPrompt $prompt) : string
    {
        ...
        return \implode(\PHP_EOL, $this->scrollbar(\array_map(function ($label, $key) use($prompt) {
            ...
            $selected = $prompt->isList() ? \in_array($label, $prompt->value()) : \in_array($key, $prompt->value());
            return match (\true) {
        ...
        }, $prompt->visible(), \array_keys($prompt->visible())), $prompt->firstVisible, $prompt->scroll, \count($prompt->matches()), \min($this->longest($prompt->matches(), padding: 4), $prompt->terminal()->cols() - 6)));
    }
    /**
