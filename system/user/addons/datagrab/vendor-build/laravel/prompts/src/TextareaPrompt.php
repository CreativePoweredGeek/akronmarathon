...
use Closure;
use BoldMinded\DataGrab\Dependency\Laravel\Prompts\Support\Utils;
class TextareaPrompt extends Prompt
{
     ...
     */
    public function __construct(public string $label, public string $placeholder = '', public string $default = '', public bool|string $required = \false, public mixed $validate = null, public string $hint = '', int $rows = 5, public ?Closure $transform = null)
    {
        $this->scroll = $rows;
        $this->initializeScrolling();
        $this->trackTypedValue(default: $default, submit: \false, allowNewLine: \true);
    ...
    {
        return $this->mbWordwrap($this->value(), $this->width, \PHP_EOL, \true);
    }
    /**
     * The formatted lines.
     *
    ...
    public function visible() : array
    {
        ...
        return \array_slice(\explode(\PHP_EOL, $withCursor), $this->firstVisible, $this->scroll, preserve_keys: \true);
    }
    /**
    ...
    {
        return \implode(\PHP_EOL, \array_map($this->dim(...), \explode(\PHP_EOL, $this->addCursor($this->mbWordwrap($this->placeholder, $this->width, \PHP_EOL, \true), cursorPosition: 0))));
    }
}
