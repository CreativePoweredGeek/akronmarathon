...
class SearchPromptRenderer extends Renderer implements Scrolling
{
    ...
    protected function renderOptions(SearchPrompt $prompt) : string
    {
        ...
        return \implode(\PHP_EOL, $this->scrollbar(\array_values(\array_map(function ($label, $key) use($prompt) {
            $label = $this->truncate($label, $prompt->terminal()->cols() - 10);
            $index = \array_search($key, \array_keys($prompt->matches()));
            return $prompt->highlighted === $index ? "{$this->cyan('›')} {$label}  " : "  {$this->dim($label)}  ";
        }, $visible = $prompt->visible(), \array_keys($visible))), $prompt->firstVisible, $prompt->scroll, \count($prompt->matches()), \min($this->longest($prompt->matches(), padding: 4), $prompt->terminal()->cols() - 6)));
    }
    /**
