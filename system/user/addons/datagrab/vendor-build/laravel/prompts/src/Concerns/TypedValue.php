...
use BoldMinded\DataGrab\Dependency\Laravel\Prompts\Key;
trait TypedValue
{
    /**
     ...
     */
    protected function trackTypedValue(string $default = '', bool $submit = \true, ?callable $ignore = null, bool $allowNewLine = \false) : void
    {
        ...
        $this->on('key', function (string $key) use($submit, $ignore, $allowNewLine) : void {
            if ($key !== '' && ($key[0] === "\x1b" || \in_array($key, [Key::CTRL_B, Key::CTRL_F, Key::CTRL_A, Key::CTRL_E]))) {
    ...
    protected function addCursor(string $value, int $cursorPosition, ?int $maxWidth = null) : string
    {
        ...
        [$truncatedBefore, $wasTruncatedBefore] = \mb_strwidth($before) > $spaceBefore ? [$this->trimWidthBackwards($before, 0, $spaceBefore - 1), \true] : [$before, \false];
        ...
        [$truncatedAfter, $wasTruncatedAfter] = \mb_strwidth($after) > $spaceAfter ? [\mb_strimwidth($after, 0, $spaceAfter - 1), \true] : [$after, \false];
        return ($wasTruncatedBefore ? $this->dim('…') : '') . $truncatedBefore . $this->inverse($cursor) . ($current === \PHP_EOL ? \PHP_EOL : '') . $truncatedAfter . ($wasTruncatedAfter ? $this->dim('…') : '');
    }
    /**
