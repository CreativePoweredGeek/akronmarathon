    ...
    protected function renderOptions(SelectPrompt $prompt) : string
    {
        return \implode(\PHP_EOL, $this->scrollbar(\array_values(\array_map(function ($label, $key) use($prompt) {
            ...
            if ($prompt->state === 'cancel') {
                return $this->dim($prompt->highlighted === $index ? "› ● {$this->strikethrough($label)}  " : "  ○ {$this->strikethrough($label)}  ");
            }
            return $prompt->highlighted === $index ? "{$this->cyan('›')} {$this->cyan('●')} {$label}  " : "  {$this->dim('○')} {$this->dim($label)}  ";
        }, $visible = $prompt->visible(), \array_keys($visible))), $prompt->firstVisible, $prompt->scroll, \count($prompt->options), \min($this->longest($prompt->options, padding: 6), $prompt->terminal()->cols() - 6), $prompt->state === 'cancel' ? 'dim' : 'cyan'));
    }
    /**
