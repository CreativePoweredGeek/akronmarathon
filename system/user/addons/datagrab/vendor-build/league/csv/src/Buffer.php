...
use Closure;
use Iterator;
use BoldMinded\DataGrab\Dependency\League\Csv\Query\Constraint\Criteria;
use BoldMinded\DataGrab\Dependency\League\Csv\Query\Predicate;
use BoldMinded\DataGrab\Dependency\League\Csv\Serializer\Denormalizer;
use BoldMinded\DataGrab\Dependency\League\Csv\Serializer\MappingFailed;
use BoldMinded\DataGrab\Dependency\League\Csv\Serializer\TypeCastingFailed;
use mysqli_result;
use PDOStatement;
use PgSql\Result;
...
final class Buffer implements TabularData
{
    ...
    public function fetchColumn(int|string $index = 0) : Iterator
    {
        ...
        in_array($index, $this->header, \true) || throw InvalidArgument::dueToInvalidColumnIndex($index, 'name', __METHOD__);
    ...
    private function formatInsertRecord(array $record) : array
    {
        ...
        return $this->validateRecord(match (\true) {
    ...
    private function filterUpdateRecord(array $record) : array
    {
        ...
        return match (\true) {
            ...
            $keys !== array_filter($keys, \is_string(...)), [] !== array_diff($keys, $this->header) => throw CannotInsertRecord::triggerOnValidation('@buffer_record_validation_on_update', $record),
            default => $record,
        };
    }
    /**
    ...
    private function validateRecord(array $record) : array
    {
        ...
        foreach ($this->validators as $name => $validator) {
            \true === $validator($record) || throw CannotInsertRecord::triggerOnValidation($name, $record);
        }
