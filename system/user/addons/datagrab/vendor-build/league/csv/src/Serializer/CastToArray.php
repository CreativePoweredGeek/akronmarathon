...
namespace BoldMinded\DataGrab\Dependency\League\Csv\Serializer;

use JsonException;
use BoldMinded\DataGrab\Dependency\League\Csv\Exception;
...
final class CastToArray implements TypeCasting
{
     ...
     * @throws MappingFailed
     */
    public function setOptions(?array $default = null, ArrayShape|string $shape = ArrayShape::List, string $separator = ',', string $delimiter = ',', string $enclosure = '"', int $depth = 512, int $flags = 0, Type|string $type = Type::String, bool $trimElementValueBeforeCasting = \false, ?int $headerOffset = null) : void
    {
    ...
    public function toVariable(mixed $value) : ?array
    {
        if (null === $value) {
            return match (\true) {
                $this->isNullable, Type::Mixed->equals($this->type) => $this->default,
