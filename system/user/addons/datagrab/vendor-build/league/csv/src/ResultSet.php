...
class ResultSet implements TabularDataReader, JsonSerializable
{
    ...
    {
        yield from new MapIterator(new CallbackFilterIterator($this->records, fn(array $record): bool => isset($record[$offset])), fn(array $record) => $record[$offset]);
    }
    /**
    ...
    {
        return match (\true) {
            \false === array_search($value, $this->header, \true) => throw InvalidArgument::dueToInvalidColumnIndex($value, $type, $method),
            default => $value,
        };
    }
    /**
