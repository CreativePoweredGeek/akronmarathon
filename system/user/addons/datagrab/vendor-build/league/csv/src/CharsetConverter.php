...
class CharsetConverter extends php_user_filter
{
    ...
    public static function register() : void
    {
        ...
        in_array($filter_name, stream_get_filters(), \true) || stream_filter_register($filter_name, self::class);
    }
    /**
     ...
     * @return resource
     */
    protected static final function appendFilter(mixed $stream, int $mode, string $input_encoding = 'UTF-8', string $output_encoding = 'UTF-8') : mixed
    {
        self::register();
     ...
     * @return resource
     */
    protected static final function prependFilter(mixed $stream, int $mode, string $input_encoding = 'UTF-8', string $output_encoding = 'UTF-8') : mixed
    {
        self::register();
        $filtername = self::getFiltername($input_encoding, $output_encoding);
        /** @var resource|false $filter */
     ...
     * @return resource
     */
    protected static final function filterStream(mixed $stream) : mixed
    {
     ...
     */
    protected static final function filterEncoding(string $encoding) : string
    {
    ...
    public function onCreate() : bool
    {
        ...
        try {
            ...
            $this->skipBomSequence = \is_array($this->params) && isset($this->params[self::BOM_SEQUENCE]) && self::SKIP_BOM_SEQUENCE === $this->params[self::BOM_SEQUENCE];
     ...
     */
    protected final function encodeField(int|float|string|null $value, int|string $offset) : array
    {
    ...
    {
        self::register();
        $document->appendStreamFilterOnRead(self::getFiltername((Bom::tryFrom($document->getInputBOM()) ?? Bom::Utf8)->encoding(), $output_encoding), [self::BOM_SEQUENCE => self::SKIP_BOM_SEQUENCE]);
        return $document;
    }
}
