...
namespace BoldMinded\DataGrab\Dependency\League\Csv;

use BoldMinded\DataGrab\Dependency\Deprecated;
...
class FragmentFinder
{
    ...
    private function find(array $parsedExpression, TabularData $tabularData) : array
    {
        ...
        if (self::TYPE_ROW === $type) {
            $rowFilter = fn(array $record, int $offset): bool => [] !== array_filter($selections, fn(array $selection) => $offset >= $selection['start'] && (null === $selection['end'] || $offset <= $selection['end']));
            return [(new Statement())->where($rowFilter)->process($tabularData)];
        }
        ...
        return array_map(fn(array $selection) => (new Statement())->offset($selection['start'])->limit($selection['length'])->select(...$selection['columns'])->process($tabularData), $selections);
    }
    /**
    ...
    private function parseExpression(string $expression, TabularData $tabularData) : array
    {
        if (1 !== preg_match(self::REGEXP_URI_FRAGMENT, $expression, $matches)) {
            return ['type' => self::TYPE_UNKNOWN, 'selections' => [['selection' => $expression, 'start' => -1, 'end' => null, 'length' => -1, 'columns' => []]]];
        }
        $type = \strtolower($matches['type']);
        /** @var non-empty-array<selection> $res */
        $res = array_reduce(explode(';', $matches['selections']), fn(array $selections, string $selection): array => [...$selections, match ($type) {
        ...
        return ['type' => $type, 'selections' => $res];
    }
    /**
     * @return selection
     */
    private function parseRowSelection(string $selection) : array
    {
        ...
        return match (\true) {
            -1 === $start, null === $end => ['selection' => $selection, 'start' => $start, 'end' => $start, 'length' => 1, 'columns' => []],
            '*' === $end => ['selection' => $selection, 'start' => $start, 'end' => null, 'length' => -1, 'columns' => []],
            default => ['selection' => $selection, 'start' => $start, 'end' => $end, 'length' => $end - $start + 1, 'columns' => []],
        };
    }
    /**
     * @return selection
    ...
    private function parseColumnSelection(string $selection, TabularData $tabularData) : array
    {
        ...
        return match (\true) {
            -1 === $start, $start >= $nbColumns => ['selection' => $selection, 'start' => -1, 'end' => null, 'length' => -1, 'columns' => []],
            null === $end => ['selection' => $selection, 'start' => 0, 'end' => null, 'length' => -1, 'columns' => [$start]],
            '*' === $end, $end > $nbColumns - 1 => ['selection' => $selection, 'start' => 0, 'end' => null, 'length' => -1, 'columns' => range($start, $nbColumns - 1)],
            default => ['selection' => $selection, 'start' => 0, 'end' => $end, 'length' => -1, 'columns' => range($start, $end)],
        };
    }
    /**
    ...
    private function parseCellSelection(string $selection, TabularData $tabularData) : array
    {
        if (1 !== preg_match(self::REGEXP_CELLS_SELECTION, $selection, $found)) {
            return ['selection' => $selection, 'start' => -1, 'end' => null, 'length' => 1, 'columns' => []];
        }
        ...
        if (null === $cellEnd) {
            return ['selection' => $selection, 'start' => $cellStartRow, 'end' => null, 'length' => 1, 'columns' => [$cellStartCol]];
        }
        if ('*' === $cellEnd) {
            return ['selection' => $selection, 'start' => $cellStartRow, 'end' => null, 'length' => -1, 'columns' => range($cellStartCol, $nbColumns - 1)];
        }
        ...
        return ['selection' => $selection, 'start' => $cellStartRow, 'end' => $cellEndRow, 'length' => $cellEndRow - $cellStartRow + 1, 'columns' => range($cellStartCol, $cellEndCol > $nbColumns - 1 ? $nbColumns - 1 : $cellEndCol)];
    }
    /**
