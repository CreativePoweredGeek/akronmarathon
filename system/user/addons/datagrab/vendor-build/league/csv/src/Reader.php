...
class Reader extends AbstractCsv implements TabularDataReader, JsonSerializable
{
    ...
    {
        return match (\true) {
            null === $this->header_offset, [] !== $this->header => $this->header,
    ...
    protected function setHeader(int $offset) : array
    {
        ...
        if (0 === $offset) {
            ...
            $header = $this->removeBOM($header, !$this->is_input_bom_included ? $inputBom?->length() ?? 0 : 0, $this->enclosure);
        }
        return match (\true) {
            [] === $header, [null] === $header, [\false] === $header, [''] === $header && 0 === $offset && null !== $inputBom => throw SyntaxError::dueToHeaderNotFound($offset),
            default => $header,
        };
    }
    /**
    ...
    protected function getLastRecord(array $header) : array
    {
        ...
        $formatter = fn(array $record): array => array_reduce($this->formatters, fn(array $record, Closure $formatter): array => $formatter($record), $record);
        $record = $row;
        if ([] === $header) {
    ...
    public function getRecordsAsObject(string $className, array $header = []) : Iterator
    {
        ...
        return Denormalizer::assignAll($className, $this->combineHeader($this->prepareRecords(), $header), $header);
    }
    /**
     * @throws Exception
     */
