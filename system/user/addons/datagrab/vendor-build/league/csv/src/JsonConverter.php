...
final class JsonConverter
{
     ...
     */
    public function __construct(int $flags = 0, int $depth = 512, int $indentSize = 4, ?callable $formatter = null, int $chunkSize = 500, JsonFormat $jsonFormat = JsonFormat::Standard)
    {
        ...
        $this->jsonEncodeChunk = match ($this->format) {
            JsonFormat::Standard => fn(array $chunk): string => $prettyPrintFormatter(substr(
                json_encode($chunkFormatter($chunk), $flags, $this->depth),
                /* @phpstan-ignore-line */
                1,
                -1
            )),
    ...
    public function download(TabularDataProvider|TabularData|iterable $records, ?string $filename = null, array $header = []) : int
    {
        ...
        return $this->save(records: $records, destination: new SplFileObject('php://output', 'wb'), header: $header);
    }
    /**
    ...
    {
        $stream = match (\true) {
            $destination instanceof Stream, $destination instanceof SplFileObject => $destination,
    ...
    public function convert(TabularDataProvider|TabularData|iterable $records, array $header = []) : Iterator
    {
        ...
        if (in_array($this->format, [JsonFormat::NdJsonHeader, JsonFormat::NdJsonHeaderLess], \true)) {
    ...
    {
        return new self(flags: 0, depth: 512, indentSize: 4, formatter: null, chunkSize: 500, jsonFormat: JsonFormat::Standard);
    }
}
