...
final class SwapDelimiter extends php_user_filter
{
    ...
    {
        in_array(self::FILTER_NAME, stream_get_filters(), \true) || stream_filter_register(self::FILTER_NAME, self::class);
    }
    /**
    ...
    public static function addTo(AbstractCsv $csv, string $inputDelimiter) : void
    {
        ...
        if ($csv instanceof Reader) {
            $csv->appendStreamFilterOnRead(self::getFiltername(), ['mb_separator' => $inputDelimiter, 'separator' => $csv->getDelimiter(), 'mode' => self::MODE_READ]);
            return;
        }
        $csv->appendStreamFilterOnWrite(self::getFiltername(), ['mb_separator' => $inputDelimiter, 'separator' => $csv->getDelimiter(), 'mode' => self::MODE_WRITE]);
    }
    /**
    ...
    public static function appendTo(mixed $stream, string $inputDelimiter, string $delimiter) : mixed
    {
        ...
        $filter = Warning::cloak(fn() => stream_filter_append($stream, self::getFiltername(), params: ['mb_separator' => $inputDelimiter, 'separator' => $delimiter, 'mode' => \str_contains(stream_get_meta_data($stream)['mode'], 'r') ? self::MODE_READ : self::MODE_WRITE]));
    ...
    public static function prependTo(mixed $stream, string $inputDelimiter, string $delimiter) : mixed
    {
        ...
        /** @var resource|false $filter */
        $filter = Warning::cloak(fn() => stream_filter_append($stream, $filtername, params: ['mb_separator' => $inputDelimiter, 'separator' => $delimiter, 'mode' => \str_contains(stream_get_meta_data($stream)['mode'], 'r') ? self::MODE_READ : self::MODE_WRITE]));
    ...
    public function onCreate() : bool
    {
        ...
        [$this->search, $this->replace] = match ($mode) {
            self::MODE_READ => [\trim($this->params['mb_separator'] ?? ''), \trim($this->params['separator'] ?? '')],
            self::MODE_WRITE => [\trim($this->params['separator'] ?? ''), \trim($this->params['mb_separator'] ?? '')],
            default => ['', ''],
        };
        return !in_array('', [$this->replace, $this->search], \true);
    }
