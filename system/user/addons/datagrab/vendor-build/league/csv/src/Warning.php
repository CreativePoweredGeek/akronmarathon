...
final class Warning
{
    ...
    {
        set_error_handler(fn(int $errno, string $errstr, string $errfile, int $errline): bool => in_array($errno, [E_WARNING, E_USER_WARNING], \true) ? throw new ErrorException($errstr, 0, $errno, $errfile, $errline) : \false);
    ...
    {
        set_error_handler(fn(int $errno, string $errstr, string $errfile, int $errline): bool => in_array($errno, [E_WARNING, E_USER_WARNING], \true));
        return self::execute($callback, $arguments);
    }
    /**
