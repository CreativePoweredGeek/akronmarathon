...
class Writer extends AbstractCsv implements TabularDataWriter
{
    ...
    protected function resetProperties() : void
    {
        ...
        $this->enclosure_replace = [[$this->enclosure, $this->escape . $this->enclosure . $this->enclosure], [$this->enclosure . $this->enclosure, $this->escape . $this->enclosure]];
    }
    protected function insertRecord(array $record) : int|false
    {
        return match ($this->enclose_all) {
            self::ENCLOSE_ALL => $this->document->fwrite(implode($this->delimiter, array_map(fn($content) => $this->enclosure . $content . $this->enclosure, str_replace($this->enclosure_replace[0], $this->enclosure_replace[1], $record))) . $this->newline),
