...
use BoldMinded\DataGrab\Dependency\League\Container\Attribute\AttributeInterface;
use BoldMinded\DataGrab\Dependency\League\Container\Exception\NotFoundException;
use BoldMinded\DataGrab\Dependency\League\Container\ReflectionContainer;
use BoldMinded\DataGrab\Dependency\League\Container\ContainerAwareInterface;
use BoldMinded\DataGrab\Dependency\League\Container\DefinitionContainerInterface;
...
trait ArgumentReflectorTrait
{
    ...
    public function reflectArguments(ReflectionFunctionAbstract $method, array $args = []) : array
    {
        ...
        foreach ($params as $param) {
            ...
            if ($type instanceof ReflectionUnionType) {
                $this->throwParameterException($name, 'union', $param->getDeclaringClass()?->getName(), $method->getName(), $method instanceof ReflectionMethod ? $method->isClosure() : \false, 'Union types are not supported');
            }
            ...
            $this->throwParameterException($name, $type instanceof ReflectionNamedType ? $type->getName() : 'unknown', $param->getDeclaringClass()?->getName(), $method->getName(), $method instanceof ReflectionMethod ? $method->isClosure() : \false, 'No default value available and no type hint to resolve');
        }
    ...
    protected function resolveArgumentForNamedType(ReflectionParameter $param, ReflectionNamedType $type) : ResolvableArgumentInterface
    {
        $typeHint = $type->getName();
        if ($type->getName() === 'mixed') {
            $this->throwParameterException($param->getName(), 'mixed', $param->getDeclaringClass()?->getName(), $param->getDeclaringFunction()->getName(), $param->getDeclaringFunction()->isClosure(), 'Mixed types are not supported');
        }
        if ($param->isDefaultValueAvailable()) {
    ...
    public function throwParameterException(string $name, string $type, ?string $declaringClass = null, ?string $declaringFunction = null, bool $isClosure = \false, ?string $additionalMessage = null) : void
    {
        throw new NotFoundException(\sprintf('Unable to resolve parameter ($%s) with type (%s) in %s%s%s()%s', $name, $type, $declaringClass ? $declaringClass . '::' : '', $declaringFunction ?? '', $isClosure ? ' [closure]' : '', $additionalMessage ? ' - ' . $additionalMessage : ''));
    }
    public abstract function getContainer() : DefinitionContainerInterface;
    public abstract function getMode() : int;
    public abstract function resolveArguments(array $arguments) : array;
}
