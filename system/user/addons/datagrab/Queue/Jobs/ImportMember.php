...
class ImportMember extends AbstractJob implements ShouldQueue, ShouldBeUnique
{
    public function fire(DataGrabJobContract|QueueJobContract $job, array $payload = []): bool
    {
        ...
        $result = $importer->setup(
            ...
            $this->settings
        )->importMember($payload);

        if ($result === true) {
            $this->job->delete();
        }

        ee('datagrab:Importer')->logger->log('ImportMember PID: ' . getmypid());
